...
void g_StartOustersDownSkill( int _x, int _y , int PriceRaito)
{
	...
	// 1饭骇皑家寸雀荐器牢飘 * 皑家饭骇 * (某腐磐饭骇)^1.3 * 200
...
void	C_VS_UI_ITEM_LIST::Show()
{
	...
	if(pCurrentFocusItem != NULL)
	{
		...
		int vx;
		// Damage
		if (pCurrentFocusItem->GetMaxDamage() != -1) // damage啊 乐绰啊?
		{
		...
		// critical hit
		if (pCurrentFocusItem->GetCriticalHit() != -1)//农府飘拿 洒飘啊 乐绰啊?
		{
	...
	case M_MOVING:
		...
		for (i=0; i < loop; i++)
		{
				 ...
				 distance_y >= 0 && distance_y < m_my_grid_rect.h)
			{
				...
				if (loop == ITEM_REF_POINT_COUNT)
				{
					// item捞 grid 康开俊 绢蠢沥档 甸绢坷搁 救栏肺 困摹矫挪促.
...
void	C_VS_UI_MIXING_FORGE::Show()
{
	...
	if (gpC_base->m_p_DDSurface_back->Lock())
	{
		...
		while (g_pInventory->IsNotEnd())
		{
			...
			// p_item篮 NULL捞 馆靛矫 酒聪促. 恐衬窍搁 粮犁窍绰 巴父 Get()窍扁 锭巩捞促.
			assert(p_item);
...
//						else
			...
			//酒捞袍 俺荐 嘛阑 alphabox 箭磊绰 嘛瘤 臼绰促 历厘秦 底促啊 唱吝俊 隔酒辑 茄波锅俊 嘛绰促
...
// 捞固 眉农登绢乐促搁.. 秒家茄促.
//
// grid_start_x, grid_start_y绰 igrid 矫累痢捞促. 捞巴阑 曼炼窍咯 item
// (x, y)甫 备茄促.
//-----------------------------------------------------------------------------
bool C_VS_UI_MIXING_FORGE::Check(int grid_start_x, int grid_start_y)
{
	...
	if( p_item != NULL)
	{
		...
		/*********************************************************
			...
			TO DO : 眠饶 可记 眉农啊 歹 鞘夸 且 订 咯扁俊 眠啊
...
//		}
	...
	if( m_p_select_item[0] == NULL && m_p_select_item[1] != NULL )
	{
		...
		// 霉锅掳 酒捞袍苞 鞍篮拌凯牢瘤, 雇教啊瓷茄 炼扒牢瘤 八荤
		// 鞍篮 拌凯, 促弗 可记, 饭绢酒捞袍篮 臼凳
		if( p_item != NULL &&			
...
// 泅犁 甸绊 乐绰 酒捞袍栏肺 急琶茄 酒捞袍狼 可记阑 昏力且 荐 乐绰瘤 魄喊茄促.
bool
C_VS_UI_REMOVE_OPTION::IsCanRemoveOption_Puritas(const MItem* pItem, const MItem* pTItem)
{
	if( pItem == NULL || pTItem == NULL ||
		...
		pItem->GetItemClass() == ITEM_CLASS_MIXING_ITEM && pItem->GetItemType() != 18 &&pTItem->GetItemClass() != ITEM_CLASS_PET_ITEM && pTItem->GetItemOptionListCount() != 2 || // 老馆利栏肺 2可记吝 窍唱甫 昏力 茄促.
	...
	else if(pItem->GetItemType() == 18) // 汗炼府
	...
	// 咯扁鳖瘤 酒捞袍 努贰胶绰 八刘登菌促. 酒捞袍 鸥涝阑-_- 八刘窍搁 等促.
...
//			if(message.m_iLevelLow[RACE_SLAYER] != -1) // 饭骇肺 眉农
//			{
...
//			}
	...
	if(!mail.title.GetLength()) //力格捞 绝栏搁...府畔..
		return;
	mail.show = true;
	mail.mark = false;
	mail.open = open;
...
void	C_VS_UI_MAILBOX::AddMail(TAB_ID tab_id, DWORD id, SIZE windowSize, const char * sender, const char * title, DWORD date, bool open, int guildid ,int guildtype)
{
	...
	// 菊俊 斥档啊 坷搁 03 捞繁侥栏肺 坷搁 菊俊 0捞 绝绰单 免仿且订 03斥 捞犯霸 嘛绢拎具 窍骨肺 斥档啊 茄磊府荐搁 菊俊 0阑 嘿咯霖促.
//	if(date < 10000000)
//		// 斥档啊 茄磊府老锭
//	{
//		dateString = "0";
...
void	C_VS_UI_MAILBOX::ReadMail(TAB_ID tabID, DWORD id)
{
	...
	while(itr != endItr)
	{
		if(itr->id == id)
		{
			...
			{
				if(tabID == TAB_HELP_ID) //HelPMessage 版快
				{
...
void	C_VS_UI_FINDING_MINE::OpenBlock(int x, int y, bool bCenter)
{
	...
	else	// 哭率滚瓢 喘范阑锭
	{
		...
		else if(m_Blocks[y*m_boardSize.cx+x].num == 0)	// 箭磊啊 绝栏搁 林困 喉废甸阑 楷促
		{
			OpenBlock(x-1, y-1);
			OpenBlock(x, y-1);
			OpenBlock(x+1, y-1);
...
void	C_VS_UI_FINDING_MINE::DrawBlock(int x, int y, BLOCK_STRUCT &block)
{
	...
	{
	case BLOCKSTATUS_CLOSE:
		if(m_status == GAMESTATUS_DIE && block.num == -1)	// 瘤汾弊府扁
		{
...
void	C_VS_UI_ARROW_TILE::Show()
{
	...
	{		
		...
		if(GetMapSize() > 0 )
		{
			...
			if( m_Player.X == -1 && m_Player.Y == 0 )		// 敲饭捞绢啊 矫累 霖厚吝捞搁
			{
			...
			if( m_Player.X == GetMapSize() && m_Player.Y == GetMapSize() -1 )		// 敲贰捞绢啊 格钎 瘤痢俊 档崔沁阑 版快
			{
...
void	C_VS_UI_ARROW_TILE::ActionMove()					// Move 滚瓢 努腐矫
{
...
void	C_VS_UI_ARROW_TILE::ActionRotation(BYTE Direction)					// 0篮 哭率 1篮 坷弗率
{
	...
	if( Value != 0 )
	{
		for(int i=0;i<GetMapSize()*GetMapSize();i++)
		{
			...
			if(m_Tile[i].Direction < 0 )
				m_Tile[i].Direction += MAX_DIRECTION;
			if( temp < m_Tile[i].Direction )	// 坷弗率栏肺 档绰 版快
			{
...
//				gpC_base->SendMessage(UI_REQUEST_FORCE_DIE, 11);
	...
	m_LatestClickTime = GetTickCount() - 5000;				// -_- 5檬傈栏肺 秦初绰寸~
...
bool	C_VS_UI_QUEST_INVENTORY::CanDropToInventory(const MItem* pItem)
{
	...
	// 4俺狼 规氢阑 眉农秦具 茄促. 鞍篮 酒捞袍 鸥涝捞 乐栏搁 公瓤!
	bool	bCanDrop = true;
...
{	
	const char* description_msg[6] = {
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CLOSE_PET_INFO].GetString(), //"局肯悼拱 沥焊芒阑 摧嚼聪促.";
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CAN_ENCHANT_PET].GetString(), //"2瞒 瓷仿阑 何咯且 荐 乐嚼聪促.";
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CANNOT_ENCHANT_PET].GetString(), //"酒流篮 2瞒 瓷仿阑 何咯且 荐 绝嚼聪促."
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CAN_3RD_ENCHANT_PET].GetString(),	//"3瞒 瓷仿阑 何咯且 荐 乐嚼聪促.";	
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CANNOT_3RD_ENCHANT_PET].GetString(),	//"酒流篮 3瞒 瓷仿阑 何咯且 荐 绝嚼聪促."
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CHANGE_PET_NICKNAME].GetString()	//] = "脐狼 葱匙烙阑 函版钦聪促.";
	};
	...
	case GAMBLE_ID :
		{
			...
			else
			{
				if(!m_PetInfo.ATTACK)
				{ 
					if(m_PetInfo.CUT_HEAD && m_PetInfo.ITEM_TYPE >2) // 2瞒 脐父 3瞒瓷仿(傍拜 or 软康)捞 乐促
...
//		gpC_base->m_p_DDSurface_back->Unlock();
	...
	if(bLock || gpC_base->m_p_DDSurface_back->Lock())
	{
		...
		if(hpday > 0 && hpday <= 30)
		{
			Rect rect(0, 0, m_pC_spk.GetWidth(HP_DAY_BAR)*hpday/30, m_pC_spk.GetHeight(HP_DAY_BAR));	// 30老 钙胶
...
{	
	...
	Set(g_GameRect.right/2-window_w/2, g_GameRect.bottom/2-window_h/2, window_w, window_h);
	int close_x = w-95, close_y = h-50;
//	m_SPK.Open(HELP_MESSAGE_DESC); // 角力 秋橇 皋技瘤 捞固瘤啊 甸绢啊具茄促.
//	m_SPK.Open(SPK_BULLETIN_BOARD);// 烙矫侩 捞固瘤 
	m_pC_button_group = new ButtonGroup(this);
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(close_x, close_y, gpC_global_resource->m_pC_assemble_box_button_spk->GetWidth(C_GLOBAL_RESOURCE::AB_BUTTON_CLOSE), gpC_global_resource->m_pC_assemble_box_button_spk->GetHeight(C_GLOBAL_RESOURCE::AB_BUTTON_CLOSE), CLOSE_ID, this, C_GLOBAL_RESOURCE::AB_BUTTON_CLOSE));
...
void C_VS_UI_HELPDESC::Run(id_t id)
{
	...
	case CLOSE_ID:
		Finish();
		// 2004, 5, 6 - sobeit add start - 牢磐其捞胶 档框富 摧篮 饶 檬焊磊 荤成磐 档框富 哆霸 ^^
...
//	const MHelpMessage& message = MHelpMessageManager::Instance().getMessage(m_helpindex);
	...
	char buf[1024];
	sprintf(buf, "力格 : %s", content.c_str());
	...
	{		
		...
		if(istag)  // 霉锅掳 巩磊凯捞 '<'牢版快
		{
			tagstr = str ;
			if(isfont)// 怕农俊 "#"捞 乐阑版快
			{
			...
			else // 怕弊俊 "<" 捞 器窃登绢乐阑版快
			{
				...
				tempspk = findkey( tagstr.c_str(), "file" );     					 // spk 蔼阑 掘绢柯促.
				tempPos = findkey( tagstr.c_str(), "pos" );							 // pos 蔼阑 掘绢柯促
				tempspk += ".jpg";
				
				CDirectDrawSurface *pSurface = GetJpgFileLoading(tempspk);
				m_width = pSurface->GetWidth();										// 急琶茄 spk 狼m_midth 甫 掘绢柯促
				m_height = pSurface->GetHeight();		
				
				NullSizex = (m_width/(char_width));									// 捞固瘤俊 鞘夸鄂 傍归 拌魂
				NullSizey = (m_height/(char_height+5))+1;	
				// 捞固瘤俊 鞘夸茄 扼牢阑 拌魂
				DrawImg(m_width, m_height, tempspk.c_str(), tempPos.c_str() , max(imgcnt,line));
				imgcnt+=NullSizey;		
				// Memdc 肺..pasing 茄 单捞磐甫 啊廉客辑 靛肺快 茄促.
			}
		}
		else 
		{
			...
			if(strcmp(tempappend.c_str(),"y") == 0) // 迄飘 怕农 促澜临阑 捞绢静扁茄促
			{
				line --;
				if(line < 0) line = 0;
				tempappend = "";
...
//				m_height = pSurface->GetHeight();		
//				
//				NullSizex = (m_width/(char_width-1));									// 捞固瘤俊 鞘夸鄂 傍归 拌魂
//				NullSizey = (m_height/(char_height+5))+1;	
				// 捞固瘤俊 鞘夸茄 扼牢阑 拌魂
//				DrawImg(m_width, m_height, tempspk.c_str(), tempPos.c_str() , max(imgcnt,line));
//				imgcnt+=NullSizey;		
...
void  C_VS_UI_HELPDESC::DrawImg(int m_width, int m_height ,const char * filename,const char * pos, int linecnt)
{
	...
	int Pagewidth = 500;										// 泅力 橇府轰俊 免仿瞪 x 轿捞令
	int Pageheight = 500;										// 泅力 橇府轰俊 免仿瞪 y 轿捞令
	int blankwidth = 35;										// left 咯归 轿捞令
	int blankheight = 7;										// top 咯归 轿捞令
	int imagelinesize = 20;
	
	Rect rect;
	if(strcmp("L",pos) == 0)			// 哭率沥凯老版快
	{
		rect.x = blankheight+20;        
		rect.y = (linecnt*imagelinesize)+blankwidth-15;
	}
	
	if(strcmp("R",pos) == 0)			// 坷弗率沥凯老版快,
	{
		rect.x = Pagewidth-m_width-30 ; 
		rect.y =(linecnt*imagelinesize) +blankwidth-15;
	}

	if(strcmp("LT",pos) == 0)			// ,哭率 傈眉 
	{
		rect.x = blankheight+20;        
		rect.y = (linecnt*imagelinesize)+blankwidth-15;
	}

	if( strcmp("RT",pos)  == 0)		 //  坷弗率傈眉 
	{
		rect.x = Pagewidth-m_width+blankheight -40; 
		rect.y  =(linecnt*imagelinesize) +blankwidth-15;
	}
	
	if(strcmp("C",pos) == 0)		// 季磐老版快 
	{
...
void C_VS_UI_HELPDESC::LoadCustomstr(char * customstrfilename)
{
	...
	{
		int len = strlen(sztemp);				    	// 茄扼牢狼 辨捞甫 备茄促.
		for(int i = len ; i >= 0; i--)					// 付瘤阜扼牢狼 付瘤阜俊 /r,/n 狼 傍归阑 瘤况林扁 困秦辑
		{
...
void C_VS_UI_HELPDESC::HelpDescPasing()
{
	...
	std::string tempstr;												// 胶飘傅阑 历厘窍绰 窃荐
	std::string tempspk;												// spk 函荐
	std::string tempPos;												// 捞固瘤 困摹 
	std::string tempindex;												// spk 牢咆胶
	std::string tempappend;												// 捞绢静扁
	char *pCur = NULL;													// 茄扼牢阑 佬绢靛妨辑 历厘窍绰 函荐
	char *istag = NULL;													//怕农啊 乐绰瘤 八荤窍绰 函荐
	char *isfont = NULL;												//怕农啊 乐绰瘤 八荤窍绰 函荐
...
//			}
		...
		tempstr = sztemp;
		istag = strstr(tempstr.c_str(),"<");  //捞固瘤 怕弊老版快
		isfont = strstr(tempstr.c_str(),"#"); // 咆胶飘 怕弊

		int  num =0;
		int start = 0;
		int index = 0;
		char* pch = NULL;
		std::string digit,custom;										// 目胶盆 胶飘傅 历厘 函荐
		pch = strchr(sztemp, '%');										// '%' 甫 茫酒辑 弊镑狼 困摹甫 府畔茄促.
		szParsingData = sztemp;											// 茄扼牢阑 佬绢客辑 szpasingdata 俊 历厘茄促.
		
		while (pch!=NULL)												// '%' 啊 绝阑锭 鳖瘤 while 悼累
		{
			num = pch-(sztemp+start);									// % 唱坷扁傈鳖瘤狼 辨捞甫 备茄促/
			custom = "%";												// customstr "%" 栏肺 檬扁拳秦霖促
			int cnt = 0;
			start += (num+1);											///%啊唱坷扁傈鳖瘤狼 辨捞甫 拌加歹秦霖促.
			while(sztemp[start+cnt] >= '0' && sztemp[start+cnt] <= '9') // 促澜俊 箭磊啊 酒匆锭 鳖瘤 歹秦霖促/
			{
			...
			{
				if(num >= 0)									// 捞惑窍瘤父 empty 啊 捞惑茄蔼捞 唱客辑 绢驴荐 绝捞 size 肺 贸府沁蝶 ぱぱ
				{
...
/*		int  num =0;
		...
		while( ( pos = szParsingData.find( "%" ) ) != -1 )				// '%' 啊 绝阑嫐鳖瘤 巩磊凯阑 茫酒辰促
		{
			int cnt = 1;
			strcpy(sztemp ,szParsingData.c_str());
			while(sztemp[pos+cnt] >= '0' && sztemp[pos+cnt] <= '9') // 促澜俊 箭磊啊 酒匆锭 鳖瘤 歹秦霖促/
			cnt++;
			char digit[100];
			memset( digit, 0, 100 );
...
*/
		strcpy(sztemp,szParsingData.c_str());       // 目胶盆 胶飘傅捞 利侩等巩磊凯//
		int len = strlen(sztemp);					// 茄扼牢狼 辨捞甫 备茄促.
		for(int  i = len ; i >= 0; i--)					// 付瘤阜扼牢狼 付瘤阜俊 /r,/n 狼 傍归阑 瘤况林扁 困秦辑
		{
			if(sztemp[i] =='\r' || sztemp[i] =='\n')
		...
		if(istag)// 怕弊老版快
		{
			if(isfont) // 迄飘(祸惑) 怕弊 老版快
			{
				parsing_data.push_back(tempstr.c_str());
				tempappend = findkey(tempstr.c_str(),"a"); // 利侩等 祸惑阑 捞绢静扁茄促
				linecnt = 1;
			}
			else // 捞固瘤 免仿怕弊老版快
			{
				while(NullSizey > 0)						// 捞镑篮 捞固瘤狼 ysize 焊促 咆胶飘啊 累阑嫐
				{											// 巢篮 ysize 父怒 傍归扼牢阑 持绢霖促
					NullSizey--;
					parsing_data.push_back(" ");
					if(NullSizey == 0) break;
				}

				parsing_data.push_back(tempstr.c_str());        // 捞镑篮 怕弊 傈眉 扼牢阑 历厘茄促
				tempspk = findkey( tempstr.c_str(), "file" );    // 怕弊蔼狼 胶橇扼捞飘 匙烙 茫扁
				tempPos = findkey( tempstr.c_str(), "pos" );	// 怕弊狼 捞固瘤 困摹甫 眠免
				//tempindex = findkey( tempstr.c_str(), "index" ); // spk 狼 牢郸胶甫 茫绰促
				tempstr = "";
...
//				jpg.Open(file_path.c_str());
				...
				m_width = pJpgSurface->GetWidth();										// 急琶茄 spk 狼m_midth 甫 掘绢柯促
				m_height = pJpgSurface->GetHeight();	
			//	m_width = m_SPK.GetWidth(atoi(tempindex.c_str()));										// 急琶茄 spk 狼m_midth 甫 掘绢柯促
			//	m_height = m_SPK.GetHeight(atoi(tempindex.c_str()));									// 急琶茄 spk 狼m_heigh 甫 掘绢柯促
				
				NullSizex = (m_width/(char_width-1));									// 捞固瘤俊 鞘夸鄂 傍归 拌魂
				NullSizey = (m_height/(char_height+5))+1;								// 捞固瘤俊 鞘夸茄 扼牢阑 拌魂
//				jpg.Release();
		...
		{
			tempstr = "";
			if(strcmp(tempPos.c_str(),"L") == 0) // 哭率俊 捞固瘤啊 乐绰版快:快急菊俊 sizex 狼 傍归阑 持篮第 茄临俊 嘛阑荐 乐绰 辨捞甫 力茄茄促
			{
			...
			if(strcmp(tempPos.c_str(),"R") == 0) //坷弗率俊 捞固瘤啊 乐绰版快:茄临俊 嘛阑荐乐绰 辨捞甫 力茄
			{
				count = limitlinecnt -NullSizex;
			}

			if(strcmp(tempPos.c_str(),"C") == 0 || strcmp(tempPos.c_str(),"RT") == 0 || strcmp(tempPos.c_str(),"LT") == 0)
			{ // 吝埃 , 哭率 傈眉瞒瘤, 坷弗率 傈眉 瞒瘤 : size y 父怒父 傍归扼牢阑持绢霖促
				while(NullSizey > 0)
				{
					NullSizey--;
					parsing_data.push_back(" ");
			...
			if(strcmp(tempappend.c_str(),"y") == 0 ) // 绢奇靛 葛靛衬?酒聪衬?
			{
			...
			if(NullSizey > 0)						// 捞固瘤 啊 乐促搁 yszie 啊 巢酒乐阑版快
			{
				NullSizex = (m_width/(char_width-1)); // 傍归阑 持绢辑 临绢电  Nullsizex 农扁甫 悸泼秦霖促
			}
			else
			{
				tempPos = "";					 // 捞固瘤狼 ysize 狼 傍归扼牢捞 场唱搁 pos 怕弊甫 努府绢 秦霖促
			}

			if(pCur == NULL) // pCur 啊 厚绢乐阑锭
			{
				pCur = sztemp;  // pCur 俊.. 茄扼牢佬绢靛赴 巩磊凯阑 历厘茄促.
			}
			
			size_t linecount = strlen(pCur); // linecount 俊 茄扼牢狼 醚巩磊凯 辨捞甫 历厘茄促
			size_t cutCount = count;         // 茄臂 2byte 吝埃俊 柄瘤绰 泅惑阑 阜扁困秦辑 荤侩窍绰 函荐
			if(linecount < count)			//  巩磊凯捞 count 焊促 累篮版快
			{
				tempstr += pCur;			// 秦寸巩磊凯阑 历厘茄促
				count -= linecount;			// count 绰 促澜巩磊凯鳖瘤狼 蔼阑 啊廉客具 窃栏肺 泅力鳖瘤狼 巩磊凯辨捞甫 曽霖促
				pCur = NULL;		
				count = limitlinecnt;
			}
			else
			{
				...
				if(g_PossibleStringCut(pCur, static_cast<int>(cutCount)) == false) // 付瘤阜巩磊凯阑 炼荤秦辑 茄臂老版快 
				{
					cutCount--;  // 巩磊凯狼 箭磊甫 窍唱 临咯霖促
				}
			...
			appendtemp = tempstr.length(); // appen 葛靛老锭 促澜临俊 菊率俊 傍归阑 持虐困秦辑
			parsing_data.push_back(tempstr.c_str());
			NullSizey--; // 捞固瘤啊 乐阑版快 茄临茄临 push_back 俊 持阑嫐 付促 茄临究 曽霖促.
			tempstr = "";
		}
	}
	while(NullSizey > 0)						// 捞镑篮 捞固瘤狼 ysize 焊促 咆胶飘啊 累阑嫐
...
char* C_VS_UI_HELPDESC::findkey(const char* tagstr ,char* keyword)
{
	...
	static char Buffer[512];  // 函荐甫 static  栏肺 秦具茄促 皋葛府 魔裹阑 阜扁困秦辑 (吝夸窍促)
	memset( Buffer, 0, 512 );
		...
		case M_LB_DOUBLECLICK:
			{
				...
				// 2004, 5, 10 sobeit add start - 努腐 困摹 焊沥
				if(RACE_OUSTERS == g_eRaceInterface)
				{
					m_sub_selected.x -= 1;
					m_sub_selected.y -= 1;
				}
				// 2004, 5, 10 sobeit add end - 努腐 困摹 焊沥
...
void	C_VS_UI_NAMING::ShowButtonDescription(C_VS_UI_EVENT_BUTTON *p_button)
{
	const static char* m_other_info_button_string[8]=
	{
		...
		(*g_pGameStringTable)[UI_STRING_MESSAGE_AUTO_HIDE_OFF].GetString(),
		(*g_pGameStringTable)[UI_STRING_MESSAGE_AUTO_HIDE_ON].GetString(),
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CHANGE_PLAYER_NICKNAME].GetString(),// = "目胶乓 葱匙烙阑 函版钦聪促.";
		(*g_pGameStringTable)[UI_STRING_MESSAGE_SELECT_PLAYER_NICKNAME].GetString(),// = "急琶等 葱匙烙栏肺 函版钦聪促.";
		(*g_pGameStringTable)[UI_STRING_MESSAGE_SHOW_HELP].GetString(),// = "档框富阑 壕聪促..";
	};
	switch(p_button->GetID())
	{
	case CLOSE_ID :
...
void	C_VS_UI_NAMING_CHANGE::ShowButtonDescription(C_VS_UI_EVENT_BUTTON *p_button)
{
	...
	{
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CLOSE_WINDOW].GetString(),
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CHANGE_PLAYER_NICKNAME].GetString(),// = "蜡历 磊蜡 葱匙烙阑 函版钦聪促.";.";
		(*g_pGameStringTable)[UI_STRING_MESSAGE_ADD_PLAYER_NICKNAME].GetString(),// = "葱匙烙阑 眠啊 钦聪促".";
		(*g_pGameStringTable)[UI_STRING_MESSAGE_CHANGE_PET_NICKNAME].GetString(),// = "脐狼 葱匙烙阑 函版钦聪促.".";
	...
	case CHANGE_ID:
		if(m_PenItem)
		{
			if(m_PenItem->GetItemType() == 22 || m_PenItem->GetItemType() == 25) // 蜡历匙捞怪 荐沥 奇
				g_descriptor_manager.Set(DID_INFO, p_button->x+x, p_button->y+y, (void *)help_info_button_string[1],0,0);
			else if(m_PenItem->GetItemType() == 23)// 脐 匙捞怪 荐沥 奇
...
//		BYTE			bStatus;
//		BYTE			bCondition;	// 绢蠢 炼扒俊 乐绰啊 0 : Happen, 1 : Complete, 2 : Fail, 3 : Reward
//		WORD			bIndex;		// 秦寸 炼扒狼 割锅掳 element牢啊
//	};
...
// 鞘夸且锭付促 肺靛 秦辑 镜瘤...茄锅 肺靛秦初绊 镜瘤..绊刮吝..
// 老窜 梅俊 肺靛秦初绊 罗~宽 静扁肺 内爹^^;
// 积阿 秦焊聪 弊锭 弊锭 肺爹窍绰霸 亮阑淀 秦辑 荐沥..せせ
// 贸澜 府胶飘 罐阑 订 扁夯 tree甫 拌加 啊瘤绊 乐绊 吝埃吝埃 modify且订 弊锭 弊锭 佬绢 柯促.
	...
	if(NULL != TempInfo)
	{
		...
		if(NULL != pElement)
		{
			if( TempInfo->bStatus == C_VS_UI_QUEST_LIST::CAN_ACCEPT ||
				TempInfo->bStatus == C_VS_UI_QUEST_LIST::CAN_REPLAY ) // 荐青 啊瓷 捞搁
			{
				RunQuestIcon();
				...
				if(_TempAttr != NULL)
				{
					if(_TempAttr->ToInt()>0) // 磊悼 荐青捞搁
					{
						m_SelectedQuestID = TempInfo->dwQuestID;
						gC_vs_ui.SendQuestIntention(true); // 荐青窍摆窜 菩哦阑 焊辰促.
					}
				}
			}
			...
			while(itr != TempInfo->vMissionList.end())
			{
				...
				if( NULL != TempMissionElement)
				{
					...
					if(NULL != pChildElement2)
					{
						...
						if(0 == stricmp(pChildElement2->GetName().c_str(),"Time")) // 矫埃 力茄捞 乐绰 固记捞搁
						{
				...
				// modify start- condition捞 fail(2)老锭 己傍捞搁 角菩肺, 角菩搁 己傍栏肺 焊咯崔扼绊 秦辑 荐沥..擎..
...
// 扁粮俊 乐绰 涅胶飘搁 盎脚, 绝栏搁 眠啊(霸烙 吝埃俊 朝扼柯促)
bool	C_VS_UI_QUEST_MANAGER::ModifyQuestManagerInfo(void* pVoid, int nType)
{
	...
	if(NULL != TempInfo)
	{
		...
		SetSubWindowInfo(TempInfo->dwQuestID , false); // 辑宏 扩档快甫 盎脚 窍瘤父 被捞 芒阑 凯柳 臼绰促.
		ReleaseQuestXML();
		return true;
	}
	return false;
}
// 扁粮狼 qid啊 乐栏搁 扁粮 器牢磐甫 QInfo肺 官槽促.
		...
		while(itr != m_QuestListInfo.end())
		{
			...
			if(TempInfo->dwQuestID == QInfo->dwQuestID)
			{
				...
				if(pElement != NULL)
				{
					...
					if(nType == 0||nType == 1) // 0: 己傍 or 1:角菩 老锭 - 鞭炼等 眠啊..-_-;
					{
...
// 辑宏 扩档 傈眉甫 摧疽促 凯菌促 茄促.
void	C_VS_UI_QUEST_MANAGER::ToggleWindow() 
{
	if(IsAnyWindowOpened()) // 茄 涅胶飘 扩档扼档 凯啡栏搁 摧绰促
		CloseAllWinow();
	else // 凯赴霸 窍唱档 绝栏搁 促 凯绢霖促 ぞぞ
	{
		//RunAllWinow();
		RunQuestList();
		if(m_QuestItemInfo.size()>0)
...
// 角菩,己傍矫 npc dialog甫 剁款促. nType == 0 : 己傍, nType == 1 : 角菩
void	C_VS_UI_QUEST_MANAGER::RunQuestFail_or_Sucess(const XMLTree *pElement, int nType)
{
	if(pElement != NULL)
	{
		...
		if(nType == 0) // 己傍老 版快 浚爹 捞亥飘甫 焊咯拎具 茄促搁
		{
...
{
	if(0 == m_QuestExcutElement.size()) // 阶牢霸 绝栏搁 官肺 角青
	{
...
{
	if(m_QuestExcutElement.size()) // 老窜 荤捞令 眉农
	{
		std::vector<_GQuestExcuteElement*>::iterator itr = m_QuestExcutElement.begin();
...
// npc dialog 犬牢 努腐 饶俊 努扼捞攫飘俊辑 蝶肺 贸府且 青悼捞 乐绰瘤 眉农...窍靛内爹烙.
		...
		if(pElement2 != NULL)
		{
			...
			if(NULL != pElement3)
			{
				...
				if(NULL != _TempAttr)
				{
					...
					if(0 == strcmp(TempAction, "dead")) // g_pPlayer 磷绰 悼累
					{
						int TargetZoneID = 0;
...
// GCExcuteElement 朝扼 吭阑 锭 npc dialog甫 剁款促.
...
void	C_VS_UI_QUEST_LIST::Show()
{
	...
	if(g_FL2_GetDC())
	{
		...
		{
			(*g_pGameStringTable)[UI_STRING_GQUEST_CAN_NOT].GetString(),// = "荐青阂啊";
			(*g_pGameStringTable)[UI_STRING_GQUEST_CAN_ACCEPT].GetString(),// = "荐青啊瓷";
			(*g_pGameStringTable)[UI_STRING_GQUEST_DOING].GetString(),// = "荐青吝";
			(*g_pGameStringTable)[UI_STRING_GQUEST_SUCCESS].GetString(),// = "己傍";
			(*g_pGameStringTable)[UI_STRING_GQUEST_COMPLETE].GetString(),// = "肯丰";
			(*g_pGameStringTable)[UI_STRING_GQUEST_FAIL].GetString(),// = "角菩";
			(*g_pGameStringTable)[UI_STRING_GQUEST_CAN_REPLAY].GetString(),// = "犁荐青啊";
		};
...
// 涅胶飘 府胶飘甫 罐酒 柳青吝牢 巴苞 肯丰等巴甸 蝶肺 府胶飘肺 父电促.
...
#ifndef _LIB
//	char* TempStr = "老捞伙荤坷腊磨迫备绞老捞伙荤坷腊磨迫备绞";
...
// 涅胶飘狼 鸥捞撇苞 胶农赋飘, 弊府绊 荐青啊瓷 惑怕甫 技泼 茄促.
...
//			}
	...
	if(szTitle != NULL&& strlen(szTitle)>0)
		m_szTitle = szTitle;
	// status绰 C_VS_UI_QUEST_LIST狼 QUEST_LIST_STATUS 捞促.
...
{	
	...
	TempInfo->szMissionTitle = "老捞伙荤坷腊磨迫%s:%d";
	...
	TempInfo2->szMissionTitle = "官焊甫 备窍扼";
		...
		for(int i =ScrPos; i< m_QuestMissionInfo.size(); i++)
		{
			...
			if(TempInfo != NULL)
			{
				...
				if(TempInfo->dwTimeLimit)
				{
					...
					if(TempInfo->bStatus == SUCCESS || TempInfo->bStatus == FAIL) // 己傍 趣篮 角菩 老锭
					{
				...
				if(TempInfo->bStatus != 1 && TempInfo->bStatus<3) // 荐青吝..捞扒 焊咯林瘤 富扼绊 秦辑.ぞぞ
				{
					sprintf(szString,"%s", szQuestStatus[TempInfo->bStatus]);
...
//		if(TempInfo->bStatus != HIDE) // 焊咯林瘤 臼澜
//			m_QuestMissionInfo.push_back(TempInfo);
//		
//		itr++;
//	}
...
#ifndef _LIB
	m_Str = "a啊唱促扼付官 荤酒磊瞒墨鸥颇窍啊唱促扼付官荤酒磊瞒墨鸥颇窍啊唱促扼付官荤酒磊瞒墨鸥颇窍啊唱促扼付官荤酒";
#endif
}
...
void	C_VS_UI_POWER_JJANG::Show()
{
	...
	RECT rect2 = { x+20, y+190, x+215, y+310 }; // 酒捞袍 沫
	Rect rt2(rect2.left, rect2.top, rect2.right - rect2.left, rect2.bottom - rect2.top );
	//gpC_base->m_p_DDSurface_back->FillRect(&rect2, 0);
	gpC_global_resource->DrawOutBox(rt2);
	
	SetRect(&rect2, x+70, y+90, x+158, y+118 ); // 勤靛戚 沫
	rt2.Set(rect2.left, rect2.top, rect2.right - rect2.left, rect2.bottom - rect2.top );
//	RECT rect2 = { x+70, y+90, x+158, y+118 }; // 勤靛戚 沫
//	Rect rt2(rect2.left, rect2.top, rect2.right - rect2.left, rect2.bottom - rect2.top );
	gpC_base->m_p_DDSurface_back->FillRect(&rect2, 0);
	gpC_global_resource->DrawOutBox(rt2);

	SetRect(&rect2, x+20, y+65, x+230, y+125 ); //勤戚 寇胞
	rt2.Set(rect2.left, rect2.top, rect2.right - rect2.left, rect2.bottom - rect2.top );
	gpC_global_resource->DrawOutBox(rt2);

	SetRect(&rect2, x+168, y+128, x+222, y+152 ); // 穿利 器牢飘 沫
	rt2.Set(rect2.left, rect2.top, rect2.right - rect2.left, rect2.bottom - rect2.top );
	gpC_base->m_p_DDSurface_back->FillRect(&rect2, 0);
	gpC_global_resource->DrawOutBox(rt2);

	SetRect(&rect2, x+20, y+125, x+230, y+155 ); // 穿利 器牢飘 寇胞
	rt2.Set(rect2.left, rect2.top, rect2.right - rect2.left, rect2.bottom - rect2.top );
	gpC_global_resource->DrawOutBox(rt2);

	SetRect(&rect2, x+168, y+158, x+222, y+182 ); // 白喉 啊瓷  器牢飘 沫
	rt2.Set(rect2.left, rect2.top, rect2.right - rect2.left, rect2.bottom - rect2.top );
	gpC_base->m_p_DDSurface_back->FillRect(&rect2, 0);
	gpC_global_resource->DrawOutBox(rt2);

	SetRect(&rect2, x+20, y+155, x+230, y+185 ); // 白喉 啊瓷 器牢飘 寇胞
	rt2.Set(rect2.left, rect2.top, rect2.right - rect2.left, rect2.bottom - rect2.top );
	gpC_global_resource->DrawOutBox(rt2);
	...
	if(m_GambleMode)
	{
		...
		SetRect(&rect2, x+29, y+197 + (TempPos)*35, x+205, y+230 + (TempPos)*35 ); // 白喉 啊瓷 器牢飘 寇胞
...
void	C_VS_UI_POWER_JJANG::ShowButtonDescription(C_VS_UI_EVENT_BUTTON *p_button)
{
	const static char* m_other_info_button_string[5]=
	{
		...
		(*g_pGameStringTable)[UI_STRING_MESSAGE_SHOW_ALPHA_WINDOW].GetString(),
		(*g_pGameStringTable)[UI_STRING_MESSAGE_SHOW_NO_ALPHA_WINDOW].GetString(),
		(*g_pGameStringTable)[UI_STRING_POWER_JJANG_GET_POINT_HELP].GetString(),//] = "颇况炉 器牢飘甫 啊廉可聪促.";
		(*g_pGameStringTable)[UI_STRING_POWER_JJANG_EXCHANGE_HELP].GetString(),//] = "颇况炉 器牢飘肺 酒捞袍阑 背券钦聪促.";
	};
	switch(p_button->GetID())
	{
	case CLOSE_ID :
...
//
// Window image狼 pixel捞 (x, y) 茄 痢捞扼档 老摹窍搁 true甫 酒聪搁 false甫 馆券茄促.
//
// Item阑 甸绊 乐阑 锭俊绰 'item 曼炼器牢飘'俊 蝶扼 柳青茄促.
//-----------------------------------------------------------------------------
...
{	
	...
	{				
		...
		//
		// Item阑 甸绊 乐栏搁 grid 困摹甫 固府 舅 荐 乐档废 茄促.
		//
		if (gpC_mouse_pointer->GetPickUpItem() && 
			m_focus_grid_x != NOT_SELECTED && 
			m_focus_grid_y != NOT_SELECTED)
...
bool C_VS_UI_INVENTORY_SUB::MouseControl(UINT message, int _x, int _y)
{
	...
	case M_LB_DOUBLECLICK:
			...
			if(gC_vs_ui.inventory_mode != 1 && !gC_vs_ui.IsRunningExchange())// 俺牢惑痢 坷锹葛靛 捞芭唱 背券芒捞 栋 乐阑 锭
			{
	...
	case M_RIGHTBUTTON_DOWN:
		// 俺牢 惑痢 葛靛, 芭贰 吝 酒匆锭, 付快胶俊 酒捞袍捞 绝阑锭
		if(gC_vs_ui.inventory_mode != 1 && !gC_vs_ui.IsRunningExchange() && NULL == gpC_mouse_pointer->GetPickUpItem()) 
		{
			...
			if(p_cur_item->GetItemClass() == ITEM_CLASS_PET_ITEM && p_cur_item->GetItemType() >2) // 2瞒 脐 家券酒捞袍 捞搁
			{
				switch(g_eRaceInterface)
				{
					case RACE_SLAYER:
...
//							g_char_slot_ingame.DOMAIN_SWORD) // 肪 40 捞窍绰 给 敬促.
							{
...
//
// 傈眉 Grid rect甫 汲沥茄促. 捞巴篮 Inventory狼 捞悼捞 乐阑 锭付促 秦拎具 茄促.
//-----------------------------------------------------------------------------
...
//
// 泅犁 Item阑 甸绊 乐栏搁 初带啊 背眉窍绊, 甸绊 乐瘤 臼栏搁 Inventory俊 乐绰
// 巴阑 笼绰促.
//
// 购啊 沁促搁 true甫, 弊犯瘤臼栏搁 false甫 馆券茄促.
//
// grid_start_x, grid_start_y绰 inventory grid 矫累痢捞促. 捞巴阑 曼炼窍咯 item
// (x, y)甫 备茄促.
//-----------------------------------------------------------------------------
bool C_VS_UI_INVENTORY_SUB::Click(int grid_start_x, int grid_start_y)
{
	...
	if (pMouseItem) // 甸绊 乐绰啊?
	{
//		if(pMouseItem->GetItemClass() == ITEM_CLASS_PET_ITEM ) // 夸扒 淬俊 昏力]
//			return false;
		// 眠啊瞪 荐 绝绰 版快
		// 促澜狼 酒捞袍篮 钢萍蒲俊 眠啊且 荐 绝促. by bezz
		ITEM_CLASS ItemClass = pMouseItem->GetItemClass();
		TYPE_ITEMTYPE ItemType = pMouseItem->GetItemType();
		if ( ( ItemClass == ITEM_CLASS_RELIC )								// 己拱
			|| ( ItemClass == ITEM_CLASS_BLOOD_BIBLE )						// 乔狼 己辑
			|| ( ItemClass == ITEM_CLASS_CASTLE_SYMBOL )					// 己 惑隆
			|| ( ItemClass == ITEM_CLASS_WAR_ITEM )							// 傈里 酒捞袍/ 靛贰帮 酒捞
			|| ( ItemClass == ITEM_CLASS_EVENT_ITEM && ItemType == 27 )		// 标惯

			// sjheon 2004.04.28 add
			|| ( ItemClass == ITEM_CLASS_EVENT_ETC && ItemType == 18 )		// 菩剐府 内牢
			|| ( ItemClass == ITEM_CLASS_EVENT_ITEM && (ItemType >= 32 && ItemType <= 36) )		// 浅急 赣府鹅 
			// sjheon 2004.04.28 add
							
			|| ( ItemClass == ITEM_CLASS_SWEEPER ) )						// 胶困欺
		{
			return false;
		}
		// by bezz
		...
		if (m_MultiPackItem->CanReplaceItem(gpC_mouse_pointer->GetPickUpItem(),		// 眠啊且 item
			m_focus_grid_x, m_focus_grid_y,	// 眠啊且 困摹 
			p_old_item))								// 盔贰乐带 item
		{
			...
			else
			{
				...
				if(g_pDXInput->KeyDown(DIK_LSHIFT))
				{
					...
					if(p_item != NULL && g_pInventory->GetFitPosition((MItem *)p_item, point))
					{
						...
						// 醚俊 藕芒阑 尝快绰 巴苞 鞍篮 巴捞 insert item捞促.
						// 困摹啊 肯傈洒 老摹且版快俊父 眠啊茄促.
						if (p_item->IsInsertToItem( p_cur_item ) && p_cur_item->GetGridX() == point.x && p_cur_item->GetGridY() == point.y)
						{
							// 捞傈俊 乐带 item俊 眠啊瞪 荐 乐绰 版快
							
							// 措惑 Item苞 甸绊 乐绰(眠啊且 Item)阑 Client俊辑 舅酒具 茄促.
							// 甸绊 乐绰 Item篮 Client俊辑 access且 荐 乐栏骨肺 措惑 Item阑 焊辰促.
							gpC_base->SendMessage(UI_ITEM_INSERT_FROM_INVENTORY,
								point.x, point.y,
								(void *)p_cur_item); // 措惑 Item
						}
						else
						{	
							// 眠啊瞪 荐 绝绰 版快
							...
							if (g_pInventory->CanReplaceItem((MItem *)p_item,		// 眠啊且 item
								point.x, point.y,	// 眠啊且 困摹 
								p_old_item))								// 盔贰乐带 item
							{
