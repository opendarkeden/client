...
BOOL APICheck::GetWsAddr()
{
	...
	memcpy(&m_bSaveSend[0], (void*)m_hsend,5);
	(FARPROC&)m_hrecv=GetProcAddress(m_hws32,"recv");
	memcpy(&m_bSaveRecv[0], (void*)m_hrecv,5);
...
// 	   CloseHandle(hProc);
	...
	BYTE	bReadCode[37];
	if (m_hsend!=NULL)	{
	  memcpy(&bReadCode[0], (void*)m_hsend,37);
	  int i;
...
// 				::ExitProcess(0);
	...
	for (int i=0;i<3;i++)
	{
	  g_ppProceAddress[i]= (DWORD_PTR)GetProcAddress(LoadLibrary(g_szCheckDLL[i*2]),g_szCheckDLL[i*2+1]);
