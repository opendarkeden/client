...
StartTitleLoading()
{
//	ifstream	titleFile;//(FILE_SSPRITE_IMAGEOBJECT, ios::binary);
...
//	titleFile.close();
//
//	ifstream	titleAniFile;//(FILE_SSPRITE_IMAGEOBJECT, ios::binary);
...
//	}
	...
	if (g_pTitleLoadingSprite==NULL)
	{
		...
		CFileIndexTable	FIT;
		ifstream indexFile(g_pFileDef->getProperty("FILE_SPRITEINDEX_UI").c_str(), ios::binary);
		FIT.LoadFromFile( indexFile );
		indexFile.close();

		ifstream spkFile(g_pFileDef->getProperty("FILE_SPRITE_UI").c_str(), ios::binary);
		if(g_MyFull)
			spkFile.seekg( FIT[5] );
		else
...
PrepareLoadingAddonSPK()
{
	...
	if (g_AddonSPKNum==0)
	{
		ifstream	AddonFileIndex2;//(FILE_ISPRITE_ADDON, ios::binary);
...
//	gC_vs_ui.SetVersion(CLIENT_VERSION);
	...
	//#ifdef OUTPUT_DEBUG
	//	ofstream indexTable(FILE_INDEXTABLE, ios::binary);
	//	CIndexSprite::SaveIndexTableToFile( indexTable );
	///	indexTable.close();
	//#endif
	
	//ifstream indexTable2;//(FILE_INFO_USINGCOLORSET, ios::binary);
	...
	ofstream file("test.ispr", ios::binary);
	is.SaveToFile( file );
	file.close();
...
{	
	...
	//---------------------------------------------------------------------
	ifstream gameStringTableTable;//(FILE_INFO_gameStringTable, ios::binary);
	...
	//---------------------------------------------------------------------
	ifstream gameStringTableTable2;//(FILE_INFO_gameStringTable, ios::binary);
	...
	{	
		...
		*/
		...
		/*
			...
			ifstream guildInfoFile2(FILE_INFO_GUILD_INFO_MAPPER, ios::binary | ios::nocreate);	
			...
			if (!guildInfoFile2.is_open())
			{
				...
				ofstream guildInfoFile(FILE_INFO_GUILD_INFO_MAPPER, ios::binary);	
...
InitGameObject()
{
	...
	int version;
	ifstream versionFile;//(g_pFileDef->getProperty("FILE_INFO_ACTION").c_str(), ios::binary);
