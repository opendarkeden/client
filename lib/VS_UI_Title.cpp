...
#define LOGIN_ID_X 75 // 상대값
#define LOGIN_ID_Y 65
#define LOGIN_PASSWORD_X 75
#define LOGIN_PASSWORD_Y 125
...
// 
//-----------------------------------------------------------------------------
C_VS_UI_CHAR_DELETE::C_VS_UI_CHAR_DELETE(bool bl_vampire)
{
	...
	if (bl_vampire)
	{
//		m_pC_image_spk = new C_SPRITE_PACK(SPK_CHAR_DELETE_VAMPIRE);
		ok_offset_x = 189-26-4;
		ok_offset_y = w_h-60;
		cancel_offset_x = 253-24-4;
...
void C_VS_UI_CHAR_DELETE::Run(id_t id)
{
	switch (id)
	{
		case DELETE_OK:
			...
			// 제대로 입력하였는가?
			if (m_lev_ssn_part1.Size() >= 1 &&
				 m_lev_ssn_part2.Size() >= 1
...
bool C_VS_UI_CHAR_DELETE::MouseControl(UINT message, int _x, int _y)
{
	...
	Rect first( 48, 107, 90, 20 );
	Rect second( 167,109, 90, 20 );
	...
	case M_LEFTBUTTON_DOWN:
	case M_LB_DOUBLECLICK:
		if( first.IsInRect(_xpos, _ypos ) )
		{
			m_lev_ssn_part2.Unacquire();
			m_lev_ssn_part1.Acquire();
		}
		else if ( second.IsInRect( _xpos, _ypos ) )
		{
			m_lev_ssn_part1.Unacquire();
...
void C_VS_UI_CHAR_DELETE::KeyboardControl(UINT message, UINT key, long extra)
{
	...
	// digit only
//	if (message == WM_CHAR && (key >= '0' && key <= '9'))
	{
...
//	DeleteNew(m_p_slot->sz_name);
	...
	S_new_character.bl_slayer	= !m_p_slot->bl_vampire;
...
//	Set(0, 0, m_pC_back.GetWidth(), m_pC_back.GetHeight());
	...
	//face back, next
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(454, 463, m_image_spk.GetWidth(FACE_BACK_BUTTON), m_image_spk.GetHeight(FACE_BACK_BUTTON), FACE_BACK_ID, this, FACE_BACK_BUTTON));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(485, 463, m_image_spk.GetWidth(FACE_NEXT_BUTTON), m_image_spk.GetHeight(FACE_NEXT_BUTTON), FACE_NEXT_ID, this, FACE_NEXT_BUTTON));

	//slayer, vampire
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(404, 224, m_image_spk.GetWidth(MALE_BUTTON)+20, m_image_spk.GetHeight(MALE_BUTTON), SLAYER_ID, this, SLAYER_BUTTON));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(474, 224, m_image_spk.GetWidth(FEMALE_BUTTON)+20, m_image_spk.GetHeight(FEMALE_BUTTON), VAMPIRE_ID, this, VAMPIRE_BUTTON));

	//male, female
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(407, 263, m_image_spk.GetWidth(MALE_BUTTON)+20, m_image_spk.GetHeight(MALE_BUTTON), MALE_ID, this, MALE_BUTTON));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(472, 264, m_image_spk.GetWidth(FEMALE_BUTTON)+20, m_image_spk.GetHeight(FEMALE_BUTTON), FEMALE_ID, this, FEMALE_BUTTON));

	//save,load,reroll
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(572, 215, m_image_spk.GetWidth(SAVE_BUTTON), m_image_spk.GetHeight(SAVE_BUTTON), SAVE_ID, this, SAVE_BUTTON));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(617, 215, m_image_spk.GetWidth(LOAD_BUTTON), m_image_spk.GetHeight(LOAD_BUTTON), LOAD_ID, this, LOAD_BUTTON));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(664, 215, m_image_spk.GetWidth(REROLL_BUTTON), m_image_spk.GetHeight(REROLL_BUTTON), REROLL_ID, this, REROLL_BUTTON));

	//check button
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(503, 178, m_image_spk.GetWidth(CHECK_BUTTON), m_image_spk.GetHeight(CHECK_BUTTON), CHECK_ID, this, CHECK_BUTTON));
	...
	m_pC_vampire_ispk.Init(num, CDirectDraw::Is565());
	...
	if (!file_vampire)
		_Error(FILE_OPEN);
	m_pC_slayer_cfpk.LoadFromFile(file_man);
	m_pC_vampire_cfpk.LoadFromFile(file_vampire);
	file_man.close();
	file_vampire.close();
...
void C_VS_UI_NEWCHAR::ShowButtonWidget(C_VS_UI_EVENT_BUTTON * p_button)
{
	...
	if(p_button->GetID() == MALE_ID || p_button->GetID() == FEMALE_ID || p_button->GetID() == SLAYER_ID || p_button->GetID() == VAMPIRE_ID)
	{
	...
	else
	{
		if(p_button->GetFocusState())
		{
			if(!m_p_slot->bl_vampire || !(p_button->GetID() == LOAD_ID || p_button->GetID() == SAVE_ID || p_button->GetID() == REROLL_ID || p_button->GetID() == FACE_BACK_ID || p_button->GetID() == FACE_NEXT_ID))
			{
				if (p_button->GetPressState()) // push state
...
//	m_p_slot->skin_color = 494;
	...
	m_p_slot->left_color = 0;
	m_p_slot->right_color = 0;
	m_p_slot->helmet_color = 0;
...
void	C_VS_UI_NEWCHAR::ShowCharacter(int _x, int _y, S_SLOT * p_slot, int index, int enable, int dark)
{
	...
	if (p_slot->bl_vampire)
	{
		if (gpC_base->m_p_DDSurface_back->Lock())
		{
			...
			index %= m_pC_vampire_cfpk[coat][enable][2].GetSize();
			...
			point.x = _x+m_pC_vampire_cfpk[coat][enable][2][index].GetCX();
			point.y = _y+m_pC_vampire_cfpk[coat][enable][2][index].GetCY();
			
			const int sprite_id = m_pC_vampire_cfpk[coat][enable][2][index].GetSpriteID();
			
			AssertVampireSprite(sprite_id);
			
			if(dark)
				gpC_base->m_p_DDSurface_back->BltIndexSpriteDarkness(&point, &m_pC_vampire_ispk[sprite_id], DARK_BIT);
			else
				gpC_base->m_p_DDSurface_back->BltIndexSprite(&point, &m_pC_vampire_ispk[sprite_id]);
	...
	{			
		...
		const int female_plus = p_slot->bl_female*m_pC_slayer_cfpk.GetSize()/2;
		
		if(p_slot->bl_female && enable == 0)
			index = index%(m_pC_slayer_cfpk[female_plus][enable][2].GetSize()-20)+20;
		else
			index %= m_pC_slayer_cfpk[female_plus][enable][2].GetSize();
		
		
		int sprite_id = 0, part_id = 0;
		
		part_id = info->trouser +female_plus;
		sprite_id = m_pC_slayer_cfpk[part_id][enable][2][index].GetSpriteID();
		
		AssertSlayerSprite(sprite_id);
		
		point.x = _x+m_pC_slayer_cfpk[part_id][enable][2][index].GetCX();
		point.y = _y+m_pC_slayer_cfpk[part_id][enable][2][index].GetCY();
		
		if(dark)
			gpC_base->m_p_DDSurface_back->BltIndexSpriteDarkness(&point, &m_pC_slayer_ispk[sprite_id], DARK_BIT);
		else
			gpC_base->m_p_DDSurface_back->BltIndexSprite(&point, &m_pC_slayer_ispk[sprite_id]);
...
//	DrawCover(ENERGY_X, 482);
	...
	if(m_p_slot->bl_vampire)
		m_image_spk.Blt(480,229, MALE_CHECK);
	else
		m_image_spk.Blt(411,229, MALE_CHECK);

	if(m_p_slot->bl_female)
		m_image_spk.Blt(473,268, MALE_CHECK);
	else
		m_image_spk.Blt(411,268, MALE_CHECK);
...
//			break;
		...
		case DELETE_3_ID:
			...
			if (m_slot[i].bl_set == true)
			{
				...
				m_pC_char_delete = new C_VS_UI_CHAR_DELETE(m_slot[i].bl_vampire);
...
//	m_pC_login_menu_default.Open(SPK_LOGIN_MENU_DEFAULT);
	...
	Set(/*154, 180*/400 - m_pC_login_spk->GetWidth()/2, 300 - m_pC_login_spk->GetHeight()/2, m_pC_login_spk->GetWidth(), m_pC_login_spk->GetHeight());

	m_pC_button_group = new ButtonGroup(this);

	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(85, 204, m_pC_login_menu.GetWidth(NEW_ID), m_pC_login_menu.GetHeight(NEW_ID), NEW_ID, this, PUSHED_NEW_ID, 1));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(85, 166, m_pC_login_menu.GetWidth(OK), m_pC_login_menu.GetHeight(OK), OK, this, PUSHED_OK, 1));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(183, 225, m_pC_login_menu.GetWidth(CANCEL), m_pC_login_menu.GetHeight(CANCEL), CANCEL, this, PUSHED_CANCEL, 1));
	...
	m_lev_id.SetAllowHangul( true );
	m_lev_password.SetAllowHangul( true );
...
//	m_pC_ani_symbol->SetSpeed(70);
	...
	m_pC_button_group = new ButtonGroup(this);
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(56, 518, m_title_menu_default.GetWidth(CONNECT), m_title_menu_default.GetHeight(CONNECT), CONNECT, this));
//	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(324+17, 278, m_title_menu_default.GetWidth(TUTORIAL), m_title_menu_default.GetHeight(TUTORIAL), TUTORIAL, this));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(238, 518, m_title_menu_default.GetWidth(OPTION), m_title_menu_default.GetHeight(OPTION), OPTION, this));
//	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(347+17, 319, m_title_menu_default.GetWidth(INTRO), m_title_menu_default.GetHeight(INTRO), INTRO, this));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(442, 518, m_title_menu_default.GetWidth(CREDIT), m_title_menu_default.GetHeight(CREDIT), CREDIT, this));
	m_pC_button_group->Add(new C_VS_UI_EVENT_BUTTON(633, 518, m_title_menu_default.GetWidth(EXIT), m_title_menu_default.GetHeight(EXIT), EXIT, this));
...
void C_VS_UI_TITLE::ShowButtonWidget(C_VS_UI_EVENT_BUTTON * p_button)
{
	if (p_button->GetFocusState() && p_button->GetPressState())
	{
		m_title_menu_select.BltLocked(p_button->x, p_button->y+2, p_button->GetID());
	}
	else
	{
...
//		gpC_base->Print(5, 5, C_VS_UI::m_sz_version, gpC_base->GREEN);
	...
	if(gpC_base->m_p_DDSurface_back->Lock())
	{
		if (m_bl_credit)
		{
			...
			int scroll3 = ( (m_credit_scroll < 0)? 300+m_credit_scroll : 300 );
		...
		else
		{
			...
			const int title_x = 189, title_y = 8;
			const int title_frame = (m_credit_scroll%300)/3;
			if(title_frame < m_pC_title_ani.GetSize())
				m_pC_title_ani.BltLocked(title_x, title_y, title_frame);
			else
				m_pC_title_ani.BltLocked(title_x, title_y);
			...
			g_PrintColorStr(750-g_GetStringWidth(sz_temp, gpC_base->m_info_pi.hfont), 470, sz_temp, gpC_base->m_info_pi, RGB_WHITE);
		}
	}
...
C_VS_UI_OPTION::C_VS_UI_OPTION()
{
	...
	m_pC_scroll_bar = new C_VS_UI_SCROLL_BAR(3, Rect(370, 112, -1, 100));
...
//	m_pC_button_group->Add( new C_VS_UI_EVENT_BUTTON(save_button_x, default_button_y, m_pC_etc_spk->GetWidth(SAVE_BUTTON_HILIGHTED), m_pC_etc_spk->GetHeight(SAVE_BUTTON_HILIGHTED), SAVE_ID, this, SAVE_BUTTON_HILIGHTED) );
	...
	// close버튼
	int close_button_x = 360, close_button_y = 285;
	m_pC_button_group->Add( new C_VS_UI_EVENT_BUTTON(close_button_x, close_button_y, m_pC_etc_spk->GetWidth(BUTTON_CLOSE), m_pC_etc_spk->GetHeight(BUTTON_CLOSE), CLOSE_ID, this, BUTTON_CLOSE) );

	// tab버튼
	int tab_control_x = 116, tab_y = 20, tab_graphic_x = 170, tab_sound_x = 224, tab_game_x = 278;
	m_pC_button_group->Add( new C_VS_UI_EVENT_BUTTON(tab_control_x, tab_y, m_pC_main_spk->GetWidth(TAB_CONTROL), m_pC_main_spk->GetHeight(TAB_CONTROL), CONTROL_ID, this, TAB_CONTROL) );
	m_pC_button_group->Add( new C_VS_UI_EVENT_BUTTON(tab_graphic_x, tab_y, m_pC_main_spk->GetWidth(TAB_GRAPHIC), m_pC_main_spk->GetHeight(TAB_GRAPHIC), GRAPHIC_ID, this, TAB_GRAPHIC) );
	m_pC_button_group->Add( new C_VS_UI_EVENT_BUTTON(tab_sound_x, tab_y, m_pC_main_spk->GetWidth(TAB_SOUND), m_pC_main_spk->GetHeight(TAB_SOUND), SOUND_ID, this, TAB_SOUND) );
	m_pC_button_group->Add( new C_VS_UI_EVENT_BUTTON(tab_game_x, tab_y, m_pC_main_spk->GetWidth(TAB_GAME), m_pC_main_spk->GetHeight(TAB_GAME), GAME_ID, this, TAB_GAME) );

	m_check_x = 125, m_check_y = 60, m_check_gap = 20;
...
//	int list_button_x = 341, list_button_y = 54;
	...
	m_pC_control_button_group->Add( new C_VS_UI_EVENT_BUTTON(m_check_x+10, m_check_y + m_check_gap, m_pC_etc_spk->GetWidth(RADIO_BACK_DISABLE), m_pC_etc_spk->GetHeight(RADIO_BACK_DISABLE), CHECK_ENTER_CHAT, this, RADIO_BACK_DISABLE) );

	for(i = 0; i < CHECK_CONTROL_MAX; i++)
		m_pC_control_button_group->Add( new C_VS_UI_EVENT_BUTTON(m_check_x, m_check_y+m_check_gap*(8+i)+20, m_pC_etc_spk->GetWidth(CHECK_BACK_DISABLE), m_pC_etc_spk->GetHeight(CHECK_BACK_DISABLE), CHECK_CONTROL_TAB+i+2, this, CHECK_BACK_DISABLE) );
	...
	for(i = 0; i < CHECK_SOUND_MAX; i++)
		m_pC_sound_button_group->Add( new C_VS_UI_EVENT_BUTTON(m_check_x, m_check_y+(2*m_check_gap)*i, m_pC_etc_spk->GetWidth(CHECK_BACK_DISABLE), m_pC_etc_spk->GetHeight(CHECK_BACK_DISABLE), CHECK_SOUND_TAB+i, this, CHECK_BACK_DISABLE) );
	m_pC_sound_button_group->Add( new C_VS_UI_EVENT_BUTTON(m_check_x+60, m_check_y+m_check_gap*4, m_pC_etc_spk->GetWidth(RADIO_BACK_DISABLE), m_pC_etc_spk->GetHeight(RADIO_BACK_DISABLE), CHECK_WAV, this, RADIO_BACK_DISABLE) );
	m_pC_sound_button_group->Add( new C_VS_UI_EVENT_BUTTON(m_check_x+120, m_check_y+m_check_gap*4, m_pC_etc_spk->GetWidth(RADIO_BACK_DISABLE), m_pC_etc_spk->GetHeight(RADIO_BACK_DISABLE), CHECK_MIDI, this, RADIO_BACK_DISABLE) );
	...
	for(i = 0; i < CHECK_GAME_MAX; i++)
	{
		...
		if( i >= 6)
			check_gap++;
		m_pC_game_button_group->Add( 
			new C_VS_UI_EVENT_BUTTON(m_check_x, m_check_y+m_check_gap*check_gap, 
			m_pC_etc_spk->GetWidth(CHECK_BACK_DISABLE), 
			m_pC_etc_spk->GetHeight(CHECK_BACK_DISABLE), 
			CHECK_GAME_TAB+i, this, CHECK_BACK_DISABLE) );
	}
...
//	m_rt_value[0].Set(m_check_x+120, m_check_y+m_check_gap*(6+CHECK_MOUSE_SPEED-CHECK_CONTROL_TAB), m_pC_etc_spk->GetWidth(VOLUME_BAR), 15);
	...
	m_rt_value[2].Set(m_check_x+120, m_check_y+m_check_gap*(CHECK_SOUND-CHECK_SOUND_TAB)+20, m_pC_etc_spk->GetWidth(VOLUME_BAR), 15);
	m_rt_value[3].Set(m_check_x+120, m_check_y+m_check_gap*(CHECK_MUSIC-CHECK_SOUND_TAB)+20+20, m_pC_etc_spk->GetWidth(VOLUME_BAR), 15);
	m_rt_value[4].Set(m_check_x+120, m_check_y+m_check_gap*(CHECK_ALPHA_DEPTH-CHECK_GRAPHIC_TAB)+20, m_pC_etc_spk->GetWidth(VOLUME_BAR), 15);
		...
		case M_MOVING :
			{
				...
				if(m_i_selected_tab == TAB_CONTROL && _x > 125 && _x < 368 && _y > 120 && _y < 102+accel_gap*7+20)
				{
					m_focus_hotkey = (_y - 120)/accel_gap;
					if(m_focus_hotkey > 6)
					{
						m_focus_hotkey = -1;
						break;
...
void C_VS_UI_OPTION::Show()
{
	...
	int i = 0, tab_x[4] = {116, 170, 224, 278}, tab_y = 20;
...
//				"Mouse Speed",
			...
			if(gpC_base->m_p_DDSurface_back->Lock())
			{
				m_pC_main_spk->BltLocked(x+m_vampire_plus_x+125, y+m_vampire_plus_y+100, HOTKEY_WINDOW);
...
//			g_PrintColorStr(x+m_vampire_plus_x+130, y+m_vampire_plus_y+56, "Input Style : ", gpC_base->m_user_id_pi, RGB_BLACK);
			...
			g_PrintColorStr(x+m_vampire_plus_x+m_check_x+30, y+m_vampire_plus_y+m_check_y+20, 
...
//			m_pC_etc_spk->Blt(x+m_vampire_plus_x+230+m_pC_etc_spk->GetWidth(HOTKEY_BACK), y+m_vampire_plus_y+53, HOTKEY_BACK_RIGHT);
			...
			for(i = 0; i < CHECK_CONTROL_MAX; i++)
				g_PrintColorStr(x+m_vampire_plus_x+m_check_x+15, y+m_vampire_plus_y+m_check_y+m_check_gap*(8+i)+20, check_string[i].c_str(), gpC_base->m_user_id_pi, RGB_BLACK);
...
//				m_pC_etc_spk->Blt(x+m_vampire_plus_x+m_rt_value[RECT_MOUSE_SPEED].x-m_pC_etc_spk->GetWidth(VOLUME_TAG)/2+m_value_mouse_speed*m_rt_value[RECT_MOUSE_SPEED].w/MAX_MOUSE_SPEED, y+m_vampire_plus_y+m_rt_value[RECT_MOUSE_SPEED].y, VOLUME_TAG);
			...
			for(i = 0; i < min(accel_count, 7); i++)
			{
				...
				g_PrintColorStr(x+m_vampire_plus_x+127, y+m_vampire_plus_y+20+80+22+accel_gap*i, pAccelName, gpC_base->m_chatting_pi, RGB_WHITE);
				WORD key = g_pKeyAccelerator->GetKey(accel);
				int px = 0;
				if(ACCEL_HAS_CONTROL(key))
					px = g_PrintColorStr(x+m_vampire_plus_x+127+100+px, y+m_vampire_plus_y+20+80+22+accel_gap*i, "Ctrl", gpC_base->m_chatting_pi, RGB_WHITE);
				if(ACCEL_HAS_ALT(key))
				{
					if(px != 0)
					{
						px = g_PrintColorStr(px, y+m_vampire_plus_y+20+80+22+accel_gap*i, "+", gpC_base->m_chatting_pi, RGB_WHITE);
						px = g_PrintColorStr(px, y+m_vampire_plus_y+20+80+22+accel_gap*i, "Alt", gpC_base->m_chatting_pi, RGB_WHITE);
					}
					else
						px = g_PrintColorStr(x+m_vampire_plus_x+127+100, y+m_vampire_plus_y+20+80+22+accel_gap*i, "Alt", gpC_base->m_chatting_pi, RGB_WHITE);
				}
				if(ACCEL_HAS_SHIFT(key))
				{
					...
					{
						px = g_PrintColorStr(px, y+m_vampire_plus_y+20+80+22+accel_gap*i, "+", gpC_base->m_chatting_pi, RGB_WHITE);
						px = g_PrintColorStr(px, y+m_vampire_plus_y+20+80+22+accel_gap*i, "Shift", gpC_base->m_chatting_pi, RGB_WHITE);
					}
					else
						px = g_PrintColorStr(x+m_vampire_plus_x+127+100, y+m_vampire_plus_y+20+80+22+accel_gap*i, "Shift", gpC_base->m_chatting_pi, RGB_WHITE);
				}
				if(px != 0)
				{
					px = g_PrintColorStr(px, y+m_vampire_plus_y+20+80+22+accel_gap*i, "+", gpC_base->m_chatting_pi, RGB_WHITE);
					px = g_PrintColorStr(px, y+m_vampire_plus_y+20+80+22+accel_gap*i, scancode_name[ACCEL_GET_KEY(key)], gpC_base->m_chatting_pi, RGB_WHITE);
				}
				else
					px = g_PrintColorStr(x+m_vampire_plus_x+127+100, y+m_vampire_plus_y+20+80+22+accel_gap*i, scancode_name[ACCEL_GET_KEY(key)], gpC_base->m_chatting_pi, RGB_WHITE);
...
//				"기합 소리 듣기",
			...
			g_FL2_GetDC();
			for(i = 0; i < CHECK_SOUND_MAX; i++)
				g_PrintColorStr(x+m_vampire_plus_x+m_check_x+15, y+m_vampire_plus_y+m_check_y+(2*m_check_gap)*i, check_string[i], gpC_base->m_user_id_pi, RGB_BLACK);
			g_PrintColorStr(x+m_vampire_plus_x+m_check_x+80, y+m_vampire_plus_y+m_check_y+m_check_gap*(i+2), "WAV", gpC_base->m_user_id_pi, RGB_BLACK);
			g_PrintColorStr(x+m_vampire_plus_x+m_check_x+140, y+m_vampire_plus_y+m_check_y+m_check_gap*(i+2), "MIDI", gpC_base->m_user_id_pi, RGB_BLACK);
			g_FL2_ReleaseDC();
...
//				"파티 가입 신청 거부",
			...
			for(i = 0; i < CHECK_GAME_MAX; i++)
			{
				...
				if (( i == 2 || i == 5 ) && ( strlen( check_string[i] ) > cutpos ))
				{
					char szTemp[256],szTemp2[256];
					ZeroMemory(szTemp, 256);
					ZeroMemory(szTemp2, 256);
					
					memcpy(szTemp, check_string[i], cutpos );
					if(check_string[i][cutpos] == ' ')
						memcpy(szTemp2, check_string[i] + cutpos+1, strlen(check_string[i]) - cutpos-1 );
					else
						memcpy(szTemp2, check_string[i] + cutpos, strlen(check_string[i]) - cutpos );

					g_PrintColorStr(x+m_vampire_plus_x+m_check_x+15, 
					y+m_vampire_plus_y+m_check_y+m_check_gap*check_gap, szTemp, 
					gpC_base->m_user_id_pi, RGB_BLACK);					
					g_PrintColorStr(x+m_vampire_plus_x+m_check_x+15, 
					y+m_vampire_plus_y+m_check_y+m_check_gap*check_gap+20, szTemp2, 
					gpC_base->m_user_id_pi, RGB_BLACK);					
				}
				else
				{
					g_PrintColorStr(x+m_vampire_plus_x+m_check_x+15, 
						y+m_vampire_plus_y+m_check_y+m_check_gap*check_gap, check_string[i], 
						gpC_base->m_user_id_pi, RGB_BLACK);
				}
			}
			g_FL2_ReleaseDC();		
