...
static const POINT g_mouse_point_fix[11] = 
{
	{0, 0},		// slayer default
	{-9, -36},	// slayer item
...
//	else frame = 0;
	...
	if (m_bl_pickup_item)
	{
		...
		gpC_item->Blt(m_mouse_x-m_half_x,
										m_mouse_y-m_half_y,
										frame_id);
	}
	else
...
//		int mouse_y = m_mouse_y + g_mouse_point_fix[m_cursor==CURSOR_SLAYER_PORTAL?CURSOR_VAMPIRE_PORTAL:m_cursor].y;
		...
		if(m_cursor == CURSOR_RESIZE && gpC_window_manager->GetMouseFocusedWindow() == m_resize_window)
		{
			int temp_cursor = gbl_vampire_interface?CURSOR_VAMPIRE_PICKUP:CURSOR_SLAYER_PICKUP;
			if(MousePushed())
		...
		else if(gpC_window_manager->GetMouseFocusedWindow())
		{
			...
			if(m_bl_description && gpC_window_manager->GetMouseFocusedWindow() != g_desc_dialog_window_id)
				...
				m_pC_mouse_pointer_spk->Blt(m_mouse_x, m_mouse_y, (gbl_vampire_interface?CURSOR_VAMPIRE_NORMAL:CURSOR_SLAYER_NORMAL));
		}
		else
		{
			if((m_cursor == CURSOR_SLAYER_PICKUP || m_cursor == CURSOR_VAMPIRE_PICKUP) && MousePushed())
			...
			if((m_cursor == CURSOR_SLAYER_PICKUP || m_cursor == CURSOR_PORTAL || 
				m_cursor == CURSOR_VAMPIRE_PICKUP) &&
				m_string != "")// || m_cursor == CURSOR_VAMPIRE_PORTAL)
			{
				RECT rect;
