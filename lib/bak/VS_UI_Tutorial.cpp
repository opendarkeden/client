...
void C_VS_UI_BOOKCASE::Show()
{
	...
	if(m_focus_book_x != NOT_SELECTED && m_focus_book_y != NOT_SELECTED)
	{
		...
		{
			int size = (*m_p_book_list)[num].size()-3;
...
//						}
		...
		case M_LB_DOUBLECLICK:
			if(m_focus_book_x != NOT_SELECTED && m_focus_book_y != NOT_SELECTED && !(*m_p_book_list).empty())
			{
				...
				m_pC_book = new C_VS_UI_BOOK((*m_p_book_list)[num].c_str(), m_book_row, m_book_col);
				m_pC_book->Start();
			}
			break;
	}
...
//			if(depth > 4)
		...
		else if(szLine[i] != '\0')
		{
			...
			if(itemClass != MItem::ITEM_CLASS_NULL && itemClass != 0)
			{
				if(	itemClass >= MItem::MAX_ITEM_CLASS) return false;

				int item_list_size = (*g_pItemTable)[(MItem::ITEM_CLASS)itemClass].GetSize();
...
int	C_VS_UI_COMPUTER::PrintTree(int _x, int _y, int y_distance, int depth, int &limit, bool close, std::vector<C_TREE_BASE> &tree)
{
	...
	int itemType = 0;
	MItem::ITEM_CLASS old_itemClass = (MItem::ITEM_CLASS)0;
	...
	for(i = 0; i < abs(limit)-1 && i < max; i++)
	{
		...
		MItem::ITEM_CLASS itemClass = (MItem::ITEM_CLASS)atoi(tree[i].str.c_str());
		...
		if(_y + (i+y_plus)*y_distance < (m_tree_limit_h+m_tree_scroll)*y_distance && _y+(i+y_plus-m_tree_scroll)*y_distance >= 0 )
		{
			...
			if(tree[i].Tree.empty())
			{
				...
				if(strcmp(pItem->GetEName(), (*g_pGameStringTable)[UI_STRING_MESSAGE_INFRA_RED_HELMET].GetString()) == 0)
				{
					char sz_temp[50];
					...
					g_PrintColorStr(x + m_index_x + _x, y + m_index_y + _y + (i+y_plus-m_tree_scroll)*y_distance, pItem->GetEName(), gpC_base->m_chatting_pi, color[depth]);	
				delete pItem;
			}
			else
	...
	for(int i = 0; i < max; i++)
	{
		...
		if(index == 0)	// 찾았다
		{
			...
			if(tree[i].Tree.empty())
			{
				...
				char sz_temp[100];
				wsprintf(sz_temp, "%s", pItem->GetEName());
