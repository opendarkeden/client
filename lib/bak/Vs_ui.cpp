...
//		DEBUG_ADD("[C_VS_UI] DIKeyboardControl FALSE");
	...
	{
#ifdef OUTPUT_DEBUG
//		DEBUG_ADD("[C_VS_UI] DIKeyboardControl Control");
#endif
		return;
	}
	...
	else
	{
		...
		if(accel !=0 && 
			(
				...
				(accel != ACCEL_CHAT && accel != ACCEL_GUILD_CHAT && accel != ACCEL_ZONE_CHAT && accel != ACCEL_WHISPER) // 채팅 단축키가 아니고,
			)
		)
		{
...
void ShowItem()
{
	...
	for (int j = 0; j < g_item_show_h; j++)
		for (int i = 0; i < g_item_show_w; i++)
		{
			...
			if (pItem!=NULL)
			{
				data_id_t id = pItem->GetInventoryFrameID(); // frame id = data id
...
// 설명창을 띄운다.
//-----------------------------------------------------------------------------
void C_VS_UI::RunDescDialog(int type, void *ptr)
{
	if (m_pC_game)
		m_pC_game->RunDescDialog(type, ptr);
}
...
//
//-----------------------------------------------------------------------------
void C_VS_UI::RunTeamList(bool ready)
{
	if (m_pC_game)
		m_pC_game->RunTeamList(ready);
}
...
//
//-----------------------------------------------------------------------------
bool	C_VS_UI::IsRunningTeamList() const
{
	if (m_pC_game)
		return m_pC_game->IsRunningTeamList();
...
//
//-----------------------------------------------------------------------------
void C_VS_UI::RunTeamInfo(bool ready, void *info)
{
	if (m_pC_game)
		m_pC_game->RunTeamInfo(ready, info);
}
...
#ifndef _LIB
	g_pMoneyManager->SetMoney(989900000);
#endif
...
// Enchant할래?하고 묻는다.
//-----------------------------------------------------------------------------
void C_VS_UI::RunEnchant()
{
	if (m_pC_game)
		m_pC_game->RunEnchant();
}
