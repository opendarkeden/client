...
////////////////////////////////////////////////////////////////////////
bool	C_VS_UI_DESC::LoadDesc(const char *szFilename, int row, int col, bool bl_title)
{
	...
	if(!m_pack_file.Open(szFilename))return false;
	...
	while(m_pack_file.GetString(szLine, dSTRING_LEN))
	{
		...
		if(temp_string[0] == '%')//그림삽입 그림 태그
		{
			...
			if(m_ori_string.empty() && !m_rep_string.empty())//item인경우
			{
				...
				temp->Init( 1, CDirectDraw::Is565() );		// 임시로 1개만 loading.. - -;;
				
				bool re = temp->LoadFromFileSprite( 0, spr_id, SPK_ITEM, SPKI_ITEM );
				assert(re);
