...
private:
	...
	int				m_tag_height, m_button_height;
	bool			m_bl_reverse;


public:
	C_VS_UI_SCROLL_BAR(int pos_max, const Rect scroll_rect, bool bl_reverse = false, const char *scroll_bar_spk_name = NULL, int bar_plus = 6, int tag_plus = 6, int button_plus = 0)
	{
		m_spk = NULL;
	...
	bool	MouseControl(UINT message, int _x, int _y)
	{
		...
		case M_LB_DOUBLECLICK:
			...
			else if(_x > x && _x < x+w && _y > y && _y < y+h)
			{
				SetScrollPixel(_y);
				m_tag_pushed = true;
				return false;
	...
	void	SetScrollPixel(int _y)
	{
		SetScrollPos((_y-y-m_tag_height/2)*m_pos_max/(h-m_tag_height));
	}
