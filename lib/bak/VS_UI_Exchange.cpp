...
bool C_VS_UI_EXCHANGE::Check(int grid_start_x, int grid_start_y)
{
	...
	if (p_item != NULL && p_item->GetItemClass() != MItem::ITEM_CLASS_RELIC
...
void	C_VS_UI_EXCHANGE::Run(id_t id)
{
	...
	switch (id)
	{
		...
		case MONEY_ID:
			...
			// 교환할 돈에서 뺀다
			DeleteNew(m_pC_dialog_withdraw_money);
			if (g_pTradeManager->GetMyMoneyManager()->GetMoney() > 0)
			{
...
void	C_VS_UI_EXCHANGE::ShowButtonWidget(C_VS_UI_EVENT_BUTTON * p_button)
{
	...
	else if(p_button->GetID() == MONEY_ID || p_button->GetID() == INVENTORY_MONEY_ID)
	{
...
//글자 출력용 dc를 잡는다.
	...
	g_Print(x+m_inventory_money_button_point.x+30, y+m_inventory_money_button_point.y+2, money_buf, &gpC_base->m_money_pi);
