...
C_VS_UI_PROGRESS::C_VS_UI_PROGRESS()
{
	...
	static bool Check[PROGRESS_MAX+7];
...
//	if(g_pUserInformation->IsNetmarble)
	...
	else
	{
		...
		m_pC_character->Init(2, CDirectDraw::Is565());
		m_pC_character->LoadFromFileSprite(0, num*2, SPK_PROGRESS_CHARACTER, SPKI_PROGRESS_CHARACTER);
		m_pC_character->LoadFromFileSprite(1, num*2+1, SPK_PROGRESS_CHARACTER, SPKI_PROGRESS_CHARACTER);
		...
		static char *Monster[PROGRESS_MAX] =
		{
			...
			(*g_pGameStringTable)[UI_STRING_MESSAGE_MONSTER_NAME_TNDEADESTROIDER].GetString(),
		...
		std::string filename = Monster[num];
		filename += ".txt";
...
void C_VS_UI_PROGRESS::Show()
{
	...
	if(gpC_base->m_p_DDSurface_back->Lock())
	{
		if(m_pC_event_progress != NULL)	// EVENT_PROGRESS
		{
			// 하드코딩
			Rect rect;
			m_pC_event_progress->BltLocked(0, 0, 0);
			rect.Set(0, m_pC_event_progress->GetHeight(1)*(100-m_percent)/100, m_pC_event_progress->GetWidth(1), m_pC_event_progress->GetHeight(1)*(m_percent)/100);
			m_pC_event_progress->BltLockedClip(0, 0, rect, 1);
		}
		else
		{
			//BACK찍기
