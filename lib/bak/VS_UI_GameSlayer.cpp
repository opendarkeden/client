...
C_VS_UI_SLAYER_GEAR::C_VS_UI_SLAYER_GEAR()
{
	...
	m_slot_rect[SN_RIGHTRING1].Set(10, 275, 30, 30);
	m_slot_rect[SN_RIGHTRING2].Set(45, 275, 30, 30);
	m_slot_rect[SN_LEFTRING1].Set(146, 275, 30, 30);
	m_slot_rect[SN_LEFTRING2].Set(181, 275, 30, 30);
...
{
	return ((*g_pSkillManager)[*(int*)arg1].GetDomainExp() < (*g_pSkillManager)[*(int*)arg2].GetDomainExp());
}
			...
			// EXP 는 버튼이 없다
			{
				...
				//			if(gpC_base->m_p_DDSurface_back->Lock())
				{
					...
					if(g_pExperienceTable->GetSTRInfo(g_char_slot_ingame.STR_PURE).GoalExp)
					{
						...
						rect.Set(0, 0, m_pC_main_spk->GetWidth(EXP_BAR)*(g_char_slot_ingame.STR_EXP_CUR-cur_value)/(g_pExperienceTable->GetSTRInfo(g_char_slot_ingame.STR_PURE).GoalExp), m_pC_main_spk->GetHeight(EXP_BAR));
					...
					if(g_pExperienceTable->GetDEXInfo(g_char_slot_ingame.DEX_PURE).GoalExp)
					{
						...
						rect.Set(0, 0, m_pC_main_spk->GetWidth(EXP_BAR)*(g_char_slot_ingame.DEX_EXP_CUR-cur_value)/(g_pExperienceTable->GetDEXInfo(g_char_slot_ingame.DEX_PURE).GoalExp), m_pC_main_spk->GetHeight(EXP_BAR));
					...
					if(g_pExperienceTable->GetINTInfo(g_char_slot_ingame.INT_PURE).GoalExp)
					{
						...
						rect.Set(0, 0, m_pC_main_spk->GetWidth(EXP_BAR)*(g_char_slot_ingame.INT_EXP_CUR-cur_value)/(g_pExperienceTable->GetINTInfo(g_char_slot_ingame.INT_PURE).GoalExp), m_pC_main_spk->GetHeight(EXP_BAR));
					...
					for(i = 0; i < 3; i++)
					{
						...
						const int exp = (*g_pSkillManager)[domain[i]].GetDomainExp();
						int prev_exp;
						if(domain_level == 0)
						{
							prev_exp = 0;
						...
						{
							prev_exp = (*g_pSkillManager)[domain[i]].GetExpInfo(domain_level-1).AccumExp;
						}
						const int next_exp = (*g_pSkillManager)[domain[i]].GetExpInfo(domain_level).AccumExp;
						
						//도메인 경험치 바
						m_pC_main_spk->BltLocked(x+bar_x, y+bar_y+bar_gap*(3+i), EXP_BACK);
						rect.Set(0, 0, m_pC_main_spk->GetWidth(EXP_BAR)*(exp - prev_exp)/(next_exp - prev_exp), m_pC_main_spk->GetHeight(EXP_BAR));
...
void C_VS_UI_SLAYER_PORTAL::Show()
{
	...
	flag_animation++;
	if(flag_animation >= 7*flag_frame)flag_animation%=7*flag_frame;
...
//		map_max = 4;
	...
	if(m_etc_spk.IsPixel(gpC_mouse_pointer->GetX()-p_button->x, gpC_mouse_pointer->GetY()-p_button->y, p_button->GetID()))
	{
		...
		if(ButtonID==CLOSE)
			...
			g_descriptor_manager.Set(DID_INFO, p_button->x, p_button->y, (void *)m_help_string[(m_map-1+map_max)%map_max],0,0);
		else if(ButtonID==RIGHT)
			g_descriptor_manager.Set(DID_INFO, p_button->x, p_button->y, (void *)m_help_string[(m_map+1)%map_max],0,0);
	}	
}
