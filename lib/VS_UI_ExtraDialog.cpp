							...
							C_VS_UI_DIALOG(_x, _y, center_x, center_y, exec_fp, dd_button)
{
	...
	InitString();
	char *PartyName[20] = {(*g_pGameStringTable)[UI_STRING_MESSAGE_DESC_PARTY_NAME].GetString(), (*g_pGameStringTable)[UI_STRING_MESSAGE_DESC_PARTY_NAME].GetString()};
...
//		break;
		...
		case ASK_PARTY_REQUEST :
		{
			...
			m_sz_question_msg_temp[0] = new char [strlen(m_sz_question_msg[type][0])+strlen(pName)+5];
			sprintf(m_sz_question_msg_temp[0], m_sz_question_msg[type][0], pName, PartyName[gbl_vampire_interface?1:0]);
			SetMessage(m_sz_question_msg_temp, 1, SMO_NOFIT);	
		}
		break;
...
//		m_iSprite = // g_npc_num[i];	// + GUARD
	...
	if(m_iSprite != -1)
	{
		m_p_NPCFace.Init( 2, CDirectDraw::Is565() );		// ÀÓ½Ã·Î 1°³¸¸ loading.. - -;;

		bool re;
		re = m_p_NPCFace.LoadFromFileSprite( 0, 0, SPK_NPCFACE, SPKI_NPCFACE );
		assert(re);
		re = m_p_NPCFace.LoadFromFileSprite( 1, m_iSprite, SPK_NPCFACE, SPKI_NPCFACE );
		assert(re);
...
// 
//-----------------------------------------------------------------------------
C_VS_UI_DESC_DIALOG::C_VS_UI_DESC_DIALOG(id_t type, void* void_ptr)
					:C_VS_UI_DESC()
{
...
//						ifstream file(SPK_VAMPIRE_SYS_BUTTON, ios::binary | ios::nocreate);
//						if (!file)
//							_Error(FILE_OPEN);
...
//
//						temp = new CSpritePack;
//						ifstream file2(SPK_VAMPIRE_CHAT_BUTTON, ios::binary | ios::nocreate);
//						if (!file2)
//							_Error(FILE_OPEN);
//						temp->LoadFromFile( file2 );
//						file2.close();
//						m_pC_inpicture.push_back(temp);
					}
					else
					{
						filename = "MainSlayer.txt";
//						temp = new CSpritePack;
//						ifstream file(SPK_SLAYER_SYS_BUTTON, ios::binary | ios::nocreate);
//						if (!file)
//							_Error(FILE_OPEN);
//						temp->LoadFromFile( file );
//						file.close();
...
//
//						temp = new CSpritePack;
//						ifstream file2(SPK_SLAYER_CHAT_BUTTON, ios::binary | ios::nocreate);
//						if (!file2)
//							_Error(FILE_OPEN);
//						temp->LoadFromFile( file2 );
//						file2.close();
...
//			filename  = TXT_ITEMROOT;
			...
			wsprintf(sz_temp, "%s", e_name.c_str() );
#endif
...
//			}
			...
			{				
				...
				while(itr != optionList.end() && *itr < g_pItemOptionTable->GetSize())
				{
					...
					char pPartName[20];
#ifdef __ENGLISH_VERSION__
					strcpy(pPartName,g_pItemOptionTable->ITEMOPTION_PARTENAME[optionInfo.Part]);
#else
			...
			if(!p_item->IsVampireItem() && gbl_vampire_interface)
			{
				...
				strcat(sz_temp, (*g_pGameStringTable)[UI_STRING_MESSAGE_DESC_ONLY_SLAYER].GetString());
				temp_string = sz_temp;
				m_rep_string.push_back(temp_string);
			}
			else if(!p_item->IsSlayerItem() && !gbl_vampire_interface)
			{
				...
				strcat(sz_temp, (*g_pGameStringTable)[UI_STRING_MESSAGE_DESC_ONLY_VAMPIRE].GetString());
				temp_string = sz_temp;
				m_rep_string.push_back(temp_string);
			}
	...
	case DID_SKILL:
		{
			...
			CSpritePack *temp;
			temp = new CSpritePack;
			temp->Init( 1, CDirectDraw::Is565() );		// ÀÓ½Ã·Î 1°³¸¸ loading.. - -;;

			int id = (int)void_ptr;
			int sprID = (*g_pSkillInfoTable)[id].GetSpriteID();

			bool re = temp->LoadFromFileSprite( 0, sprID, SPK_SKILL_ICON, SPKI_SKILL_ICON );
			assert(re);
			...
			SetDescTitle((*g_pSkillInfoTable)[id].GetName());
		}
		break;
...
//		SetSprite(0, sprID, 0);
	...
	if(LoadDesc(filename.c_str(), 60, 17, bl_title)==false)Run(CLOSE_ID);
...
void C_VS_UI_FILE_DIALOG::Start(char *type)
{
	...
	if(gbl_vampire_interface)
	{
//		m_p_image_spk = new C_SPRITE_PACK(SPK_VAMPIRE_DESC);
	}
	else
	{
//		m_p_image_spk = new C_SPRITE_PACK(SPK_SLAYER_DESC);
	}
...
void	C_VS_UI_FILE_DIALOG::Show()
{
	...
	g_PrintColorStr(x+40, y+5,name,gpC_base->m_desc_menu_pi, RGB_WHITE);
