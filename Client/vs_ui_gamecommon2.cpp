...
void g_StartOustersDownSkill( int _x, int _y )
{
	...
	wsprintf(szCost,"%d",
		int ( float((*g_pSkillInfoTable)[g_CurrentSkillID].LevelUpPoint) * pow(g_char_slot_ingame.level, 1.3 ) * 200.0f   ) );
...
// 
//-----------------------------------------------------------------------------
C_VS_UI_REQUEST_RESURRECT::C_VS_UI_REQUEST_RESURRECT(bool resurrect, bool elixir, bool eternity)
{
	...
	case M_MOVING:
		...
		for(i = 0 ;i < RESURRECT_MODE_MAX; i ++ )
		{
			if( m_ResurrectButton[i].m_Enable == true )
			{
				...
				{
					m_focus = i;
					g_descriptor_manager.Set(DID_INFO, x, y, (void*)szMsgList[i],0,0);
					break;
				}
			}
		}				
	...
	if( p_item != NULL)
	{
		...
		**********************************************************/
		...
		{// ¾Æ¿ì½ºÅÍÁî Á¤·É¼®ÀÌ ¾Æ´Ò ¶§ ¿É¼Ç Ã¼Å© 
			if( !IsCorrectType( p_item->GetItemType() ))
			msg = UI_STRING_MESSAGE_CANNOT_MIXING_ITEM_TYPE;
			else		
			if(m_p_select_item[0] == NULL && m_p_select_item[1] != NULL &&
	...
	if( m_p_select_item[0] == NULL && m_p_select_item[1] == NULL )
	{
		...
		if (p_item != NULL &&
			...
			p_item->GetItemClass() == ITEM_CLASS_OUSTERS_WRISTLET ||
			 IsCorrectType(p_item->GetItemType())
			 )
			)
		{			
...
bool C_VS_UI_MIXING_FORGE::IsCorrectType(TYPE_ITEMTYPE item_type)
{
	switch( m_forge_type )
	{
	case TYPE_A:
...
C_VS_UI_REMOVE_OPTION::IsCanRemoveOption_Puritas(const MItem* pItem, const MItem* pTItem)
{
	if( pItem == NULL || pTItem == NULL ||
		pTItem->IsQuestItem() || pTItem->IsUniqueItem() || pTItem->GetItemClass() != ITEM_CLASS_PET_ITEM && pTItem->GetItemOptionListCount() != 2 ||
		!(pItem->GetItemClass() == ITEM_CLASS_MIXING_ITEM && pItem->GetItemType() >= 9 && pItem->GetItemType() <= 17))
		return false;
	...
	enum { CLASS_WEAPON, CLASS_ARMOR, CLASS_ACCESSORY };	
...
//	AttrTopmost( true );
	...
	m_pC_scroll_bar = new C_VS_UI_SCROLL_BAR(0, Rect(240, 180, -1, 70));
...
void	C_VS_UI_OUSTERS_SKILL_INFO::Show()
{
	...
	if(g_FL2_GetDC())
	{
		g_PrintColorStr(x+30, y+35, sInfo.GetHName(), gpC_base->m_chatting_pi, RGB_WHITE);
...
//			sprintf(szTemp, (*g_pGameStringTable)[UI_STRING_MESSAGE_SKILL_LEVEL].GetString(), exp_level);					
		...
		switch(status)
		{
			case MSkillDomain::SKILLSTATUS_LEARNED:
				{
					...
					if( m_bDownSkill )
					{
						g_PrintColorStr(x+40, y+122, (*g_pGameStringTable)[UI_STRING_MESSAGE_DESC_DOWN_SKILL].GetString(), gpC_base->m_chatting_pi, color);
												
						sprintf(szTemp,(*g_pGameStringTable)[UI_STRING_MESSAGE_WITHDRAW_POINT].GetString(), sInfo.LevelUpPoint );
						g_PrintColorStr(x+40, y+138, szTemp, gpC_base->m_chatting_pi, RGB_WHITE);
					} else
					{
						g_PrintColorStr(x+40, y+122, (*g_pGameStringTable)[UI_STRING_MESSAGE_SKILL_LEVEL_UP].GetString(), gpC_base->m_chatting_pi, color);
			...
			default:
				...
				sprintf(szTemp, (*g_pGameStringTable)[UI_STRING_MESSAGE_SKILL_LEVEL].GetString(), exp_level);
				g_PrintColorStr(x+40, y+120, (*g_pGameStringTable)[UI_STRING_MESSAGE_LEARN_SKILL].GetString(), gpC_base->m_chatting_pi, color);
				sprintf(szTemp,(*g_pGameStringTable)[UI_STRING_MESSAGE_REQUIRE_SKILL_POINT].GetString(), sInfo.SkillPoint );
				g_PrintColorStr(x+40, y+138, szTemp, gpC_base->m_chatting_pi, color);

				sprintf(szTemp, (*g_pGameStringTable)[UI_STRING_MESSAGE_SKILL_LEVEL].GetString(), exp_level);
				break;
		}

		g_PrintColorStrShadow(x+100, y+70, szTemp, gpC_base->m_chatting_pi, color, shadow_color);
			...
			)
		{
			...
			g_PrintColorStr(x+100, y+86+line_gap, szTemp, gpC_base->m_chatting_pi, g_ELEMENTAL_COLOR[sInfo.ElementalDomain]);
		}
	...
	else if(p_button->GetID() == LEARN_ID)
	{
		...
		case MSkillDomain::SKILLSTATUS_LEARNED:
			if( m_bDownSkill )
			{
				...
				SKILLINFO_NODE sInfo = (*g_pSkillInfoTable)[m_skillID];
				if( sInfo.GetExpLevel() > 1 /*&& sInfo.GetExpLevel() < 30*/ )
				{
					bCanDown = true;
				}
...
//	};
	...
	RunGQuestExcuteElement(103,1,1);
...
void	C_VS_UI_QUEST_MANAGER::RunQuestFail_or_Sucess(const XMLTree *pElement, int nType)
{
	...
	if(pElement != NULL)
	{
		...
		if(pElement2 != NULL)
		{
			...
			m_pC_dialog = new C_VS_UI_NPC_DIALOG(ExecF_GQuestNpcScript, NpcID, Sender, 500, 250 ,DIALOG_OK);
	...
	if(pElement != NULL)
	{
		...
		if(pElement2 != NULL)
		{
			...
			if(NULL != pElement3)
			{
				...
				m_pC_dialog = new C_VS_UI_NPC_DIALOG(ExecF_GQuestNpcScript, NpcID, Sender, 500, 250 ,DIALOG_OK);
//				DIALOG_MENU d_menu[] = { {"´Ý±â", DIALOG_EXECID_EXIT},};
//				m_pC_dialog->SetMenu(d_menu, 1, false);
...
void	C_VS_UI_QUEST_DETAIL::SetQuestDetailInfo(char* szTitle, char* szDescription, int Status)
{
	if(szDescription != NULL && strlen(szDescription)>0)
	{
		...
		{
			SetDesc(7, 50+m_OustersOffset, RGB_GRAY, gpC_base->m_chatting_pi);
			m_pC_scroll_bar->SetPosMax(GetDescSize() - GetDescCol() +1);
//			if(szTitle != NULL)
//			{
