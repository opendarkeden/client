...
//					int kkk=0;
					clothes = addonInfo.FrameID;
//					if(clothesType ==ADDON_COAT)
//						kkk=1;
//						clothes = 23;	//add by viva
//					if(clothesType == ADDON_TROUSER)
//						kkk=2;
//						clothes = 24;
//					FRAME_ARRAY &FA = m_NewSlayerManFPK[clothes][action][direction];
					FRAME_ARRAY &FA = m_AddonFPK[clothes][action][direction];
...
//						CIndexSprite* pSprite = &m_NewSlayerManSPK[sprite];
						CIndexSprite* pSprite = &m_AddonSPK[ sprite ];
...
void	MTopView::DrawAdvancementClassSlayerCharacter( POINT *pPoint, MCreature* pCreature, int action, int direction, int frame, int FrameIndex )
{
	...
	else
	{
		...
		else
		{
			...
			//CCreatureFramePack& slayerFPK = pCreature->IsMale() ? m_NewSlayerManFPK : m_NewSlayerWomanFPK;
			//CIndexSpritePack& addonISPK = pCreature->IsMale() ? m_NewSlayerManSPK : m_NewSlayerWomanSPK;
