...
C_VS_UI_OUSTERS::C_VS_UI_OUSTERS():C_VS_UI_TRIBE()
{
	...
	const int button_x = 26, button_y = 76, button_x_gap = 37, button_y_gap = 37;
...
void C_VS_UI_OUSTERS::Show()
{
	if(gpC_base->m_p_DDSurface_back->Lock())
	{
		...
		// EXP 는 버튼이 없다
		{
			const int bar_x = 95, bar_y = 85, str_x = 27, num_x = 46, bar_gap = 14;
			char sz_temp[100];
			Rect rect;
...
// 			m_pC_main_spk->BltLockedClip(x+bar_x, y+bar_y, rect, OUSTERS_EXP_BAR);
			...
			g_PrintColorStrShadow(x+num_x, y+bar_y-3+bar_gap*3+8, sz_temp, gpC_base->m_chatting_pi, g_ELEMENTAL_COLOR[ITEMTABLE_INFO::ELEMENTAL_TYPE_EARTH], RGB_BLACK);
	...
	g_FL2_GetDC();				
	g_PrintColorStr(x + 130 - g_GetStringWidth(m_date.c_str(), gpC_base->m_chatting_pi.hfont)/2, y+13, m_date.c_str() ,gpC_base->m_chatting_pi, RGB_WHITE);	
	g_PrintColorStr(x + 130 - g_GetStringWidth(m_time.c_str(), gpC_base->m_chatting_pi.hfont)/2, y+35, m_time.c_str(), gpC_base->m_chatting_pi, RGB_WHITE);	
