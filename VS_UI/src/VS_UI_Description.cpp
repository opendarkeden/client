...
//				strcat(sz_ename, " "); // add 'space'
		...
		{
				//add by sonic 2006.10.28 显示三个属性装备名字
			...
			if(p_item->GetItemOptionListCount() == 2 )			// Rare 酒捞袍牢 版快
			{
...
//				}
		...
		} else
		{
			...
			// Damage
			if (p_item->GetOriginalSpeed() > 0) // damage啊 乐绰啊?
			{
				vx = g_PrintColorStr(px, py, (*g_pGameStringTable)[UI_STRING_MESSAGE_DESC_ATTACK_SPEED].GetString(), gpC_base->m_item_desc_pi, ITEM_DESC_RGB);
				sprintf(sz_buf, "???");				
				g_PrintColorStr(vx, py, sz_buf, gpC_base->m_item_desc_pi, RGB(200, 200, 255));
			...
			if(p_item->IsIdentified())
			{
				if(p_item->IsGearItem())
				{
					...
					if( RequireAdvancementLevel > 0 )
					{
						...
						// edit by Coffee  2007-5-20 修正装备显示二转等级不正问题
...
//	MString temp_string = (*g_pItemTable)[p_item->GetItemClass()][p_item->GetItemType()].Description;
	...
	if(pMouseItem != NULL && pMouseItem->GetItemClass() == ITEM_CLASS_EVENT_STAR && p_item->IsGearItem())
	{
		...
		if(pMouseItem->GetItemType() == 12)					// 喉风 滚靛绰 可记 滴俺父
		{
...
// add by Coffee 2006.11.4  修正足球为蓝宝石，并显示能充三属性装备
		if(pMouseItem->GetItemType() == 6)					// 喉风 滚靛绰 可记 滴俺父
		{
...
// end by Coffee 蓝宝石效果
		...
		// add by svi 2009-07-01 显示黄色水滴可以升级1,2属性物品
		if(pMouseItem->GetItemType() == 22 )			// 黄色水滴
		{
		...
		// add by svi 2009-07-15 属性消除宝石
		if(pMouseItem->GetItemType() == 24 )			// 月光石
		{
		...
		if(pMouseItem->GetItemType() == 25 )			// 紫鸦石
		{
		...
		if(pMouseItem->GetItemType() == 26 )			// 菱锌石
		{
			if(p_item->GetItemOptionListCount() == 3)
				bCanUpgrade = true;
			else
	...
	{		
		...
		if((pMouseItem->GetItemType() < 13 || pMouseItem->GetItemType() >= 16 && pMouseItem->GetItemType() <= 19) &&	// 12鳖瘤绰 饭靛 靛酚
...
void _Item_Description_Calculator(void (*fp_show)(Rect, void *, long, long), int x, int y, void * void_ptr, long left, long right)
{
	...
	if(pAddItem != NULL)
	{
		...
		{
			if(pAddItem->GetItemType() >= 0 && pAddItem->GetItemType() <=3) // 龙靛饭 内绢漓
